...
namespace UnifiedLogin.LandingAPI.Controllers
{
    ...
    {
        private readonly IManageProductEasyLMSAsync _manageProductEasyLMS;
        ...
        public ProductEasyLMSController(
            ...
            IManageProductEasyLMSAsync manageProductEasyLMS,
        ...
        {
            _manageProductEasyLMS = manageProductEasyLMS ?? throw new ArgumentNullException(nameof(manageProductEasyLMS));
        ...
        public async Task<IActionResult> ProductEasyLMSUrl(string userName = "", bool createUser = false, CancellationToken cancellationToken = default)
        {
            ...
            var companyMap = await _manageProductEasyLMS.GetCompanyAPICodeAndKeyAsync(userClaim, userClaim.PersonaId, userClaim.PersonaId, cancellationToken);
            ...
            if (createUser)
            {
                ...
                Result resultResponse = await GetEasyLMSResponseAsync(requestUri, cancellationToken);
            ...
            Result result = await GetEasyLMSResponseAsync(requestUri, cancellationToken);
            ...
            if (result.status == 1)
            {
                ...
                else
                {
                    ...
                    await _productRepository.UpdateProductSettingProductStatusAsync(userClaim.PersonaId, productId, "ProductStatus", (int)ProductBatchStatusType.Success, cancellationToken);
                }
            }
            else
            {
            ...
            string productName = await _manageProductEasyLMS.GetProductNameAsync(userClaim, ProductEnum.EasyLMS, cancellationToken);
            string message = string.IsNullOrEmpty(userClaim.ImpersonatedByName)
                ? $"User {userClaim.FirstName} {userClaim.LastName} accessed product {productName}."
                : $"RealPage user {userClaim.ImpersonatedByName} accessed product {productName}.";
        ...
        /// <summary>
        /// Send a GET request to the EasyLMS API and deserialize the result
        /// </summary>
        private async Task<Result> GetEasyLMSResponseAsync(string requestUri, CancellationToken cancellationToken = default)
        {
