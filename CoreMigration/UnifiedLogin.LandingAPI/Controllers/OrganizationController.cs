...
namespace UnifiedLogin.LandingAPI.Controllers
{
    ...
    public class OrganizationController : BaseController
    {
        ...
        private readonly IManageOrganizationProductAsync _manageOrganizationProduct;
        ...
        public OrganizationController(
            ...
            IManageOrganizationProductAsync manageOrganizationProduct,
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> InsertOrganization([FromBody] OrganizationCreate organization, [FromQuery] bool processBlueBookMessage = false, CancellationToken cancellationToken = default)
        {
            ...
            {
                var partnerInstance = await _manageBlueBook.GetCompanyInstanceBySourceAndInstanceIdAsync(organization.CompanyInstancePartnerSourceId, organization.CompanyInstancePartner, cancellationToken);
                if (partnerInstance == null)
                    return BadRequest("Partner instance could not be found");
            }

            var organizationDomainList = await _manageOrganization.ListOrganizationDomainAsync(cancellationToken);

            if (!organizationDomainList.Exists(d => d.Name.Equals(organization.OrganizationDomain, StringComparison.OrdinalIgnoreCase)))
            {
                RepositoryResponse response = await _manageOrganization.CreateOrganizationDomainAsync(new OrganizationDomain() { Name = organization.OrganizationDomain }, cancellationToken);
                if (response.Id > 0)
            ...
            List<string> invalidProductList = await _manageOrganization.ParseProductAsync(organization.Products, addProductList, cancellationToken);

            if (invalidProductList.Count > 0)
                return BadRequest("An invalid product was given : " + String.Join(",", invalidProductList));

            var result = await _manageOrganization.CreateOrganizationAsync(organization, addProductList, processBlueBookMessage, cancellationToken);
            ...
            try
            {
                companyMapResource = await _manageBlueBook.GetCompanyMapAsync(result.obj.Org.RealPageId, (long)organization.BooksCustomerMasterId, ProductEnumHelper.StringValueOf(ProductEnum.UnifiedPlatform), result.obj.Org.OrganizationDomain.Name, includeGreenBookCares: false, cancellationToken: cancellationToken);
            }
            catch (Exception)
            {
                // ignored
            ...
            var companyTypeName = (await _manageOrganization.ListOrganizationTypeAsync(cancellationToken))?.Find(t => t.OrganizationTypeId == organization.OrganizationTypeId)?.Name;
            ...
            if (addInstance)
            {
                var companyCreatedSuccessfully = await _manageBlueBook.AddUPFMCompanyFromCompanySetupAsync(companyInstance, cancellationToken);

                if (!companyCreatedSuccessfully)
                    return BadRequest("There was a problem adding the UPFM instance to UDM");

                if (!await _manageOrganization.AddUpdateCompanyToUnifiedSettingsAsync(companyInstance.CompanyInstanceSourceId, "Create", companyInstance.CustomerEnvironment, cancellationToken))
                ...
                IList<ProductUI> productList = await _manageProduct.GetProductsAsync(result.obj.Org.RealPageId, 0, true, cancellationToken: cancellationToken);
                foreach (var product in productList)
                {
                    var productInternalSettings = await _manageProduct.GetProductInternalSettingsAsync(product.ProductId, cancellationToken);
                    ...
                    if (updateInUDM?.Value == "1")
                    {
                        ...
                        await _manageBlueBook.ProductCenterEnableAsync(spc, cancellationToken);
                    }
                }
            }
        ...
        [ProducesResponseType((int)HttpStatusCode.NotFound)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> UpdateOrganization([FromBody] OrganizationUpdate organization, CancellationToken cancellationToken = default)
        {
            if (organization == null)
                return NotFound("Not found");

            Organization org = await _manageOrganization.GetOrganizationAsync(organization.RealPageId, cancellationToken: cancellationToken);
            CompanyLocation oldAddress = (await _manageOrganization.GetCompanyListAsync(org.Name, 0, null, 0, new Dictionary<object, object>(), cancellationToken))?.FirstOrDefault()?.CompanyLocation;
            ...
            var orgTypes = await _manageOrganization.ListOrganizationTypeAsync(cancellationToken);
            ...
            var orgDomains = await _manageOrganization.ListOrganizationDomainAsync(cancellationToken);
            ...
            org.organizationType = orgTypes?.Find(t => t.OrganizationTypeId == organization.OrganizationTypeId);
            ...
            var repositoryResponse = await _manageOrganization.UpdateOrganizationAsync(org, cancellationToken);

            if (!String.IsNullOrEmpty(repositoryResponse.ErrorMessage))
                return BadRequest(repositoryResponse.ErrorMessage);

            await _manageOrganization.UpdateOrganizationUsePrimaryPropertySettingAsync(org, cancellationToken);
            await _manageOrganization.UpdateOrganizationThirdPartyIDPAsync(org, cancellationToken);

            if (orgNameChanged || orgStatusChanged || orgAddressChanged || orgTypeChanged)
            {
                IList<CustomerCompanyMap> companyMapResource = await _manageBlueBook.GetCompanyMapAsync(org.RealPageId, org.BooksCustomerMasterId, ProductEnumHelper.StringValueOf(ProductEnum.UnifiedPlatform), organization.OrganizationDomainName, includeGreenBookCares: false, cancellationToken: cancellationToken);
                ...
                if (companyMapResource != null && companyMapResource.Any(c => c.CompanyInstanceSourceId == org.RealPageId.ToString()))
                {
                    ...
                    var companyTypeName = orgTypes?.Find(t => t.OrganizationTypeId == organization.OrganizationTypeId)?.Name;
                    ...
                    var booksResult = await _manageBlueBook.UpdateBooksGreenBookCompanyInstanceAsync(updateCompanyInstance, oldAddress, cancellationToken);
                    ...
                    if (orgStatusChanged)
                        await _manageOrganization.AddCompanyToJobAsync(org.RealPageId.ToString(), _userClaimsAccessor.UserId, _userClaimsAccessor.PersonaId, org.IsActive, cancellationToken);

                    if (!await _manageOrganization.AddUpdateCompanyToUnifiedSettingsAsync(org.RealPageId.ToString(), "Update", null, cancellationToken))
                        return BadRequest($"Unified Login and MDM company was updated successfully but Settings data update failed.");
                }
                else
                {
            ...
            org = await _manageOrganization.GetOrganizationAsync(org.RealPageId, cancellationToken: cancellationToken);
            return Ok(org);
        }
        ...
        [ProducesResponseType((int)HttpStatusCode.NotFound)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> GetOrganization(Guid? realPageId = null, CancellationToken cancellationToken = default)
        {
            if (realPageId.HasValue)
            {
                Organization org = await _manageOrganization.GetOrganizationAsync(realPageId.Value, cancellationToken: cancellationToken);
                if (org == null)
                    return NotFound("Not found");
                return Ok(org);
            }
            ...
            {
                IList<Organization> orgList = await _manageOrganization.GetOrganizationListAsync(cancellationToken);
                return Ok(orgList);
            }
        }
        ...
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> ListOrganizationByEnterpriseUserId(Guid realPageId, [FromQuery] string roleTypeFrom = null, [FromQuery] string roleTypeTo = null, [FromQuery] string relationshipType = null, CancellationToken cancellationToken = default)
        {
            if (realPageId == Guid.Empty)
                return BadRequest("Invalid parameter: realPageId");

            IList<Organization> organizationList = await _manageUserLogin.ListOrganizationByEnterpriseUserIdAsync(realPageId, relationshipType, cancellationToken);

            if (organizationList != null)
            {
                foreach (var organization in organizationList)
                {
                    var partyRelationship = await _managePartyRelationship.GetPartyRelationshipAsync(realPageId, organization.RealPageId, roleTypeFrom, roleTypeTo, relationshipType, cancellationToken);
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> GetProductsByOrganization([FromRoute] Guid? realPageId, [FromQuery] bool? mergePersonaAccess, [FromQuery] bool? allProducts, CancellationToken cancellationToken = default)
        {
            ...
            {
                var persona = await _managePersona.GetPersonaAsync(_userClaimsAccessor.PersonaId, withRights: false, cancellationToken: cancellationToken);
            ...
            CheckPasswordExpirationResponse checkPasswordExpirationResponse = await _manageCredential.CheckPasswordExpirationAsync(_userClaimsAccessor.UserId, _userClaimsAccessor.UserRealPageGuid, cancellationToken);
            if (checkPasswordExpirationResponse != null && !checkPasswordExpirationResponse.IsPasswordExpired)
            {
                ...
                IList<ProductUI> productList = await _manageProduct.GetProductsAsync(org.RealPageId, personaId, allProducts.HasValue ? allProducts.Value : false, replaceProductCodeWithUDMIfExists: false, cancellationToken: cancellationToken);
                if (allProducts.HasValue && allProducts.Value)
                    output.list = await _manageProduct.AddProductSourceAndGreenBookCareFlagToProductsAsync(org.RealPageId, org.PartyId, productList, cancellationToken);
                else
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> AddProductToOrganization([FromRoute] Guid realPageId, [FromBody] EnableDisableProducts enableDisableProducts, CancellationToken cancellationToken = default)
        {
            Status<IErrorData> errorStatus = new Status<IErrorData>();
            ...
            Organization org = await _manageOrganization.GetOrganizationAsync(realPageId, cancellationToken: cancellationToken);
            ...
            List<string> invalidProductList = await _manageOrganization.ParseProductAsync(enableDisableProducts.AddProducts, addProductList, cancellationToken);
            if (enableDisableProducts.Removeproducts != null)
                await _manageOrganization.ParseProductAsync(enableDisableProducts.Removeproducts, removeProductList, cancellationToken);
            ...
            {
                IList<ProductUI> productList = await _manageProduct.GetProductsAsync(org.RealPageId, 0, true, replaceProductCodeWithUDMIfExists: false, cancellationToken: cancellationToken);
                var repositoryResponse = await _manageOrganizationProduct.CheckSharedProductsEnabledAsync(productList, addProductList, removeProductList, cancellationToken);

                if (!string.IsNullOrEmpty(repositoryResponse.ErrorMessage))
                    return Ok(new { message = repositoryResponse.ErrorMessage });

                await _manageOrganization.EnableProductOnOtherProductsActivationAsync(addProductList, cancellationToken);
                var insertResponse = await _manageOrganizationProduct.InsertUpdateOrganizationProductAsync(org, addProductList, cancellationToken);

                if (!string.IsNullOrEmpty(((RepositoryResponse)insertResponse)?.ErrorMessage))
                    return BadRequest(new { message = ((RepositoryResponse)insertResponse).ErrorMessage });
            }
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> UpdateUsePrimaryPropertyForOrganizationProduct(long organizationPartyId, int productId, bool usePrimaryProperty, CancellationToken cancellationToken = default)
        {
            ...
            var repositoryResponse = await _manageOrganization.UpdateUsePrimaryPropertyForOrganizationProductAsync(organizationPartyId, productId, usePrimaryProperty, cancellationToken);
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> DeleteProductFromOrganization([FromRoute] Guid realPageId, [FromBody] EnableDisableProducts enableDisableProducts, CancellationToken cancellationToken = default)
        {
            Status<IErrorData> errorStatus = new Status<IErrorData>();
            ...
            List<string> invalidProductList = await _manageOrganization.ParseProductAsync(enableDisableProducts.Removeproducts, unassignProductList, cancellationToken);
            if (enableDisableProducts.AddProducts != null)
                await _manageOrganization.ParseProductAsync(enableDisableProducts.AddProducts, assignProductList, cancellationToken);
            ...
            {
                IList<ProductUI> productList = await _manageProduct.GetProductsAsync(org.RealPageId, 0, true, replaceProductCodeWithUDMIfExists: false, cancellationToken: cancellationToken);
                var repositoryResponse = await _manageOrganizationProduct.CheckSharedProductsEnabledAsync(productList, assignProductList, unassignProductList, cancellationToken);

                if (!string.IsNullOrEmpty(repositoryResponse.ErrorMessage))
                    return Ok(repositoryResponse.ErrorMessage);

                var deleteResponse = await _manageOrganizationProduct.DeleteProductsFromOrganizationAsync(unassignProductList, org, cancellationToken);
        ...
        [ProducesResponseType((int)HttpStatusCode.BadRequest)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> GetOrganizationIdentityProviderType([FromQuery] Guid? realPageId = null, CancellationToken cancellationToken = default)
        {
            ...
            IList<IdentityProviderType> identityProviderTypeList = await _manageOrganization.GetOrganizationIdentityProviderTypeAsync(realPageId.Value, cancellationToken);
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> OrganizationType(CancellationToken cancellationToken = default)
        {
            ObjectListOutput<OrganizationType, IErrorData> output = new ObjectListOutput<OrganizationType, IErrorData>();
            Status<IErrorData> errorStatus = new Status<IErrorData>();

            IList<OrganizationType> organizationTypeList = await _manageOrganization.ListOrganizationTypeAsync(cancellationToken);
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> GetOrganizationDomain(CancellationToken cancellationToken = default)
        {
            ...
            IList<OrganizationDomain> organizationDomainList = await _manageOrganization.ListOrganizationDomainAsync(cancellationToken);
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> GetCompanyList([FromQuery] string organizationName = null, [FromQuery] int? domain = null, [FromQuery] int? blueId = null, [FromQuery] int? organizationId = null, RequestParameter datafilter = null, CancellationToken cancellationToken = default)
        {
            ...
            List<CompanySetup> companyList = await _manageOrganization.GetCompanyListAsync(organizationName, domain ?? 0, blueId, organizationId ?? 0, globals, cancellationToken);
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> GetCompanyMasterByCustomerCompanyId(long customerCompanyId, CancellationToken cancellationToken = default)
        {
            if (customerCompanyId == 0)
                return BadRequest("CompanyMasterId not supplied");

            var companyMaster = await _manageOrganization.SearchCompanyDetailsByCustomerCompanyIdAsync(customerCompanyId, cancellationToken);
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> ListCompanyExport([FromQuery] string organizationName = null, [FromQuery] string domain = null, [FromQuery] int? blueId = null, [FromQuery] int? organizationId = null, [FromQuery] RequestParameter datafilter = null, [FromQuery] SaveFormat dataFormat = SaveFormat.CSV, CancellationToken cancellationToken = default)
        {
            byte[] plainBytes;
            IDictionary<object, object> globals = new Dictionary<object, object>();
            ...
            List<CompanySetup> companyList = await _manageOrganization.GetCompanyListAsync(organizationName, domainId, blueId, organizationId ?? 0, globals, cancellationToken);
            ...
            if (companyList != null)
            {
                ...
                if (errorStatus.Success)
                {
                    ...
                    output = new ObjectOutput<string, IErrorData>() { obj = Convert.ToBase64String(plainBytes), Status = errorStatus };
                    return Ok(output);
                }
                else
                {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            List<CompanyPropertySetup> companyPropertySetup = await _manageOrganization.GetPropertiesForCompanyAsync(companyInstanceId, propertyName, domain, blueId, status, globals, editorPersonaId, userPersonaId, isSelectedProperties, selectedProperties, operatorCode, operatorValue, cancellationToken);
        ...
        [ProducesResponseType((int)HttpStatusCode.BadRequest)]
        [ProducesResponseType((int)HttpStatusCode.NoContent)]
        public async Task<IActionResult> UpdateCompanyProperties(CompanyPropertyBatch batchRecord, CancellationToken cancellationToken = default)
        {
            if (batchRecord == null)
            ...
            var companyPropertySetup = await _manageOrganization.GetPropertiesForCompanyAsync(companyInstanceGuid, globals: globals, editorPersonaId: batchRecord.CreateUserPersonaId, cancellationToken: cancellationToken);
            ...
            {
                await _manageOrganization.UpdateCompanyInstanceAsync(batchRecord.CompanyBatchJobId, (int)ProductBatchStatusType.Error, "No properties found for company", cancellationToken);
                return NoContent();
            }

            if (propertyList.Any(p => p.InstanceId == Guid.Empty))
            {
                await _manageOrganization.UpdateCompanyInstanceAsync(batchRecord.CompanyBatchJobId, (int)ProductBatchStatusType.Error, "Invalid parameter: propertyInstanceId", cancellationToken);
                return BadRequest("Invalid parameter: propertyInstanceId");
            }

            if (propertyList.Any(p => string.IsNullOrWhiteSpace(p.Name)))
            {
                await _manageOrganization.UpdateCompanyInstanceAsync(batchRecord.CompanyBatchJobId, (int)ProductBatchStatusType.Error, "Null parameter: propertyName", cancellationToken);
                return BadRequest("Null parameter: propertyName");
            }

            var propertyInstanceIds = propertyList.Select(p => p.InstanceId).Distinct().ToList();
            var propertyListToUpdate = await _manageOrganization.GetPropertiesByInstanceIdAsync(propertyInstanceIds, cancellationToken);
            var oldPropertyList = await _manageOrganization.GetPropertiesByInstanceIdAsync(propertyInstanceIds, cancellationToken);
            ...
            var repositoryResponse = await _manageOrganization.UpdatePropertyListAsync(propertyListToUpdate, companyInstanceGuid, cancellationToken);

            if (repositoryResponse.Id == 0 || !string.IsNullOrEmpty(repositoryResponse.ErrorMessage))
            {
                await _manageOrganization.UpdateCompanyInstanceAsync(batchRecord.CompanyBatchJobId, (int)ProductBatchStatusType.Error, "Unable to update property list.", cancellationToken);
                return BadRequest(repositoryResponse.ErrorMessage);
            }
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    Parallel.ForEach(propertyListToUpdate, options, property =>
                    {
                        ...
                        _manageOrganization.UpdatePropertyInSettingsAndActivityLogsAsync(property, companyInstanceGuid, oldPropertyList, userClaim).GetAwaiter().GetResult();
                    });
                }
                catch
                {
                    await _manageOrganization.UpdateCompanyInstanceAsync(batchRecord.CompanyBatchJobId, (int)ProductBatchStatusType.Error, "Update property list process failed.").ConfigureAwait(false);
                }
            });

            await _manageOrganization.UpdateCompanyInstanceAsync(batchRecord.CompanyBatchJobId, (int)ProductBatchStatusType.Success, string.Empty, cancellationToken);
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> UpdatePropertyForOrganization([FromBody] List<UPFMPropertyInstance> propertyList, [FromQuery] Guid companyInstanceId, [FromQuery] bool isFromBulkPropertyUpdate = false, CancellationToken cancellationToken = default)
        {
            if (companyInstanceId == Guid.Empty)
            ...
            if (!isFromBulkPropertyUpdate)
            {
                ...
                {
                    var userClaim = _userClaimsAccessor.GetUserClaim();
                    repositoryResponse = (RepositoryResponse)await _manageOrganization.ProcessPropertyListAsync(property, companyInstanceId, userClaim);
                });
                await Task.WhenAll();
            }
            else
            {
                var userClaim = _userClaimsAccessor.GetUserClaim();
                List<UPFMPropertyInstance> oldPropertyList = await _manageOrganization.GetPropertiesByInstanceIdAsync(propertyList.Select(m => m.InstanceId).ToList(), userClaim, cancellationToken);
                repositoryResponse = await _manageOrganization.UpdatePropertyListAsync(propertyList, companyInstanceId, userClaim, cancellationToken);
                if (repositoryResponse.Id > 0)
                {
                    _ = Task.Run(() =>
                    {
                        ...
                        Parallel.ForEach(propertyList, options, property =>
                        {
                            ...
                            _manageOrganization.UpdatePropertyInSettingsAndActivityLogsAsync(property, companyInstanceId, oldPropertyList, userClaimLocal).GetAwaiter().GetResult();
                        });
                    });
                }
            }
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> UpdateUDMPropertyForOrganization([FromBody] UPFMPropertyInstance property, CancellationToken cancellationToken = default)
        {
            ...
            var companyInstances = await _manageBlueBook.GetCompanyInstancesByCustomerCompanyIdAsync(property.CustomerCompanyId, cancellationToken);
            ...
            var currentProperty = (await _manageOrganization.GetPropertyByInstanceIdAsync(property.InstanceId, cancellationToken)).FirstOrDefault();
            ...
            if (currentProperty != null && !string.IsNullOrEmpty(companyInstanceId))
            {
                ...
                var repositoryResponse = await _manageOrganization.UpdatePropertyAsync(property, new Guid(companyInstanceId), cancellationToken);
                if (repositoryResponse.Id == 0 || !string.IsNullOrEmpty(repositoryResponse.ErrorMessage))
                    return BadRequest(repositoryResponse.ErrorMessage);
            }
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> ListPropertyExport([FromQuery] Guid companyInstanceId, [FromQuery] string propertyName = null, [FromQuery] string domain = null, [FromQuery] int? blueId = null, [FromQuery] int? status = null, [FromQuery] RequestParameter datafilter = null, [FromQuery] SaveFormat dataFormat = SaveFormat.CSV, CancellationToken cancellationToken = default)
        {
            byte[] plainBytes;
            IDictionary<object, object> globals = new Dictionary<object, object>();
            ObjectOutput<string, IErrorData> output = new ObjectOutput<string, IErrorData>();
            ...
            var cacheKey = $"getPropertyInstanceForCompany_{companyInstanceId}";
            RPObjectCache.RemoveFromCache(cacheKey);
            List<CompanyPropertySetup> propertyList = await _manageOrganization.GetPropertiesForCompanyAsync(companyInstanceId, propertyName, domain, blueId, status, globals, cancellationToken: cancellationToken);
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> AddPropertyForOrganization([FromBody] UPFMPropertyInstance property, [FromQuery] Guid companyInstanceID, CancellationToken cancellationToken = default)
        {
            if (companyInstanceID == Guid.Empty)
                return BadRequest("Invalid parameter: companyInstanceID");
            ...
            var repositoryResponse = await _manageOrganization.AddPropertyForOrganizationAsync(property, companyInstanceID, cancellationToken);
            return Ok(repositoryResponse);
        }
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> DeleteProperty(Guid propertyInstanceID, Guid companyInstanceID, CancellationToken cancellationToken = default)
        {
            if (propertyInstanceID == Guid.Empty)
                return BadRequest("Invalid parameter: propertyInstanceID");

            var repositoryResponse = await _manageOrganization.DeletePropertyForOrganizationAsync(propertyInstanceID, companyInstanceID, cancellationToken);
            return Ok(repositoryResponse);
        }
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> SearchPropertyByBlueId(string customerPropertyId, [FromQuery] string booksCustomerMasterId, CancellationToken cancellationToken = default)
        {
            ...
            PropertyInstanceSearch propertySearchList = await _manageOrganization.SearchPropertyDetailsByCustomerPropertyIdAsync(customerPropertyId, booksCustomerMasterId, cancellationToken);
            return Ok(propertySearchList);
        }
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> GetProductStatusDetails(string productInstanceId, string source, CancellationToken cancellationToken = default)
        {
            ...
            ProductPropertyDetails productPropertyDetails = await _manageOrganization.GetSourceProductDetailsAsync(productInstanceId, source, cancellationToken);
            return Ok(productPropertyDetails);
        }
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> AuditCompanyProductPropertiesToUPFM(Guid companyInstanceId, int productId, CancellationToken cancellationToken = default)
        {
            if (companyInstanceId == Guid.Empty)
                return BadRequest("Company Instance Id not supplied");
            ...
            {
                var adminCreatorRealPageId = await _manageOrganization.GetOrganizationAdminUserRealPageIdAsync(companyInstanceId, cancellationToken);
                if (adminCreatorRealPageId == Guid.Empty)
                    return BadRequest("Admin Creator RealPageId is empty.");

                userClaim = await RecreateClaimsForClientAsync(adminCreatorRealPageId, userClaim, cancellationToken);
            }
            ...
            var orgDetails = await _manageOrganization.GetOrganizationAsync(companyInstanceId, cancellationToken: cancellationToken);
            ...
            var adminUserGuid = await _manageOrganization.GetOrganizationAdminUserRealPageIdAsync(orgDetails.RealPageId, cancellationToken);
            if (adminUserGuid == Guid.Empty)
                return BadRequest("Unable to locate company user");

            userClaim.UserRealPageGuid = adminUserGuid;
            var auditResult = await GetAuditProductPropertiesAsync(companyInstanceId, productId, adminUserGuid, orgDetails.PartyId, userClaim, cancellationToken);
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> AuditCompanyProductPropertiesToUPFMExport(Guid companyInstanceId, int productId, [FromQuery] SaveFormat dataFormat = SaveFormat.Csv, CancellationToken cancellationToken = default)
        {
            byte[] plainBytes;
            ObjectOutput<string, IErrorData> output = new ObjectOutput<string, IErrorData>();
            Status<IErrorData> errorStatus = new Status<IErrorData>();
            ...
            userClaim.UserRealPageGuid = adminUserGuid;
            List<PropertyAudit> propertyAudit = await GetAuditProductPropertiesAsync(companyInstanceId, productId, adminUserGuid, orgDetails.PartyId, userClaim, cancellationToken);
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> RunCompanyDatabaseDeleteAndUDMCleanUp(CancellationToken cancellationToken = default)
        {
            await _manageOrganization.DeleteQueuedOrganizationsAsync(cancellationToken);
            return Ok();
        }
        ...
        [ProducesResponseType((int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> InsertOrganizationToDelete([FromBody] OrganizationDelete orgToDelete, [FromQuery] bool removeUPFMInstanceInUDM = true, CancellationToken cancellationToken = default)
        {
            var errorList = ValidateObject(orgToDelete).ToList();
            if (errorList.Count > 0)
                return BadRequest(errorList);

            var org = await _manageOrganization.GetOrganizationAsync((Guid)orgToDelete.OrganizationRealPageId, cancellationToken: cancellationToken);
            if (org == null)
            ...
            var result = await _manageOrganization.InsertOrganizationRemovalQueueAsync(organizationRemovalQueue, cancellationToken);
        ...
        private async Task<DefaultUserClaim> RecreateClaimsForClientAsync(Guid realpageUserId, DefaultUserClaim userClaim, CancellationToken cancellationToken)
        {
            if (!string.IsNullOrEmpty(realpageUserId.ToString()))
            {
                var person = await _managePerson.GetPersonAsync(realpageUserId, cancellationToken);
                if (person == null)
                ...
                var userLogin = await _manageUserLogin.GetUserLoginOnlyAsync(realpageUserId, cancellationToken);
                var persona = await _managePersona.GetActivePersonaWithoutRightsAsync(realpageUserId, cancellationToken);
        ...
        private async Task<List<PropertyAudit>> GetAuditProductPropertiesAsync(Guid companyInstanceId, int productId, Guid adminUserGuid, long partyId, DefaultUserClaim userClaim, CancellationToken cancellationToken)
        {
            var userLogin = await _manageUserLogin.GetUserLoginAsync(adminUserGuid, partyId, cancellationToken);
            ...
            if (userLogin != null)
            {
                ...
                var userPersonas = await _manageUserLogin.GetUserPersonaOrganizationAsync(userLogin.LoginName, cancellationToken: cancellationToken);
            ...
            return await _manageOrganization.AuditCompanyProductPropertiesToUPFMAsync(companyInstanceId, productId, userClaim, cancellationToken);
        }
