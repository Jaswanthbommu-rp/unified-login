...
public class BulkUserUpdateJob(IServiceProvider serviceProvider, IOptions<BatchProcessorSettings> settings, ILogger<BulkUserUpdateJob> logger) : BackgroundService
{
    private readonly string _jobName = "BulkUserUpdateJob";

    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        var jobSettings = settings.Value.BulkUserUpdate;
    ...
    private async Task ProcessPendingBatchesAsync(JobSettings jobSettings, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var pendingBatches = await batchRepository.GetBulkUsersUpdateBatchToProcessAsync(jobSettings.BatchSize, cancellationToken);
    ...
    /// </summary>
    private async Task ProcessSingleBatchAsync(BulkUserBatch batch, IBatchRepository batchRepository, IProductApiClient apiClient, JobSettings jobSettings, CancellationToken cancellationToken)
    {
        try
        {
            logger.LogInformation("{JobName}: Processing batch {BatchId}, SubjectUserPersonaId: {SubjectUserPersonaId}, EditorUserPersonaId: {EditorUserPersonaId}", _jobName, batch.BulkUserBatchProcessId, batch.SubjectUserPersonaId, batch.EditorUserPersonaId);
            ...
            var endpointConfig = configs.FirstOrDefault(c => c.BatchProcessTypeId == batch.BatchProcessTypeId && c.ConfigurationTypeName == "BulkUserProcessApiEndpoint");

            // Call the API
            var response = await apiClient.ProcessBulkUserBatchAsync(batch, endpointConfig.Value, cancellationToken);

            logger.LogInformation("{JobName}: Batch {BatchId} processed with status: {Status}", _jobName, batch.BulkUserBatchProcessId, response);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "{JobName}: Error processing batch {BatchId}", _jobName, batch.BulkUserBatchProcessId);
        }
    }
}
