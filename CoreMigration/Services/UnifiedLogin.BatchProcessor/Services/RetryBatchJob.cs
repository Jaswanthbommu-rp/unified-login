...
/// <summary>
/// Background service for processing retry batch records
/// </summary>
public class RetryBatchJob(IServiceProvider serviceProvider, IOptions<BatchProcessorSettings> settings, ILogger<RetryBatchJob> logger) : BackgroundService
{
    private readonly string _jobName = "RetryBatchJob";

    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        var jobSettings = settings.Value.RetryBatch;
    ...
    private async Task ProcessPendingBatchesAsync(JobSettings jobSettings, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var pendingBatches = await batchRepository.GetBatchToProcessAsync(jobSettings.BatchSize, shouldIncludeErrorRecords: true, retryCount: 3, cancellationToken);
