...
/// <summary>
/// Background service for processing enterprise roles records with improved parallel processing.
///
/// Improvements:
...
/// </summary>
public class EnterpriseRolesJob(
    IServiceProvider serviceProvider,
    IOptions<BatchProcessorSettings> settings,
    ILogger<EnterpriseRolesJob> logger,
    BatchProcessingMetrics metrics) : BackgroundService
{
    ...
    /// <summary>
    /// Processes enterprise roles batch records with improved error handling and performance.
    /// </summary>
    private async Task ProcessPendingBatchesAsync(JobSettings jobSettings, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            IList<EnterpriseRoleBatch> pendingBatches;
            ...
            using (var fetchScope = serviceProvider.CreateScope())
            {
                ...
                pendingBatches = await batchRepository.GetEnterpriseRoleProductUpdateBatchToProcessAsync(
                ...
                configLookup = configs
                    .Where(c => c.ConfigurationTypeName == "EnterpriseRoleProcessApiEndpoint")
            ...
            await Parallel.ForEachAsync(pendingBatches, parallelOptions, async (batch, ct) =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    logger.LogError(ex, "{JobName}: Critical error processing batch {BatchId}. Continuing with remaining batches.",
                        _jobName, batch.EnterpriseRoleBatchProcessId);

                    // ✅ Try to log error for tracking (no status update method for this batch type)
                    logger.LogError(ex, "{JobName}: Failed to process batch {BatchId}. Error: {ErrorMessage}",
                        _jobName, batch.EnterpriseRoleBatchProcessId, ex.Message);
                }
            });
    ...
    /// <summary>
    /// Processes a single enterprise roles batch record with rate limiting and resilience.
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            logger.LogInformation("{JobName}: Processing batch {BatchId}, EnterpriseRoleTemplateId: {EnterpriseRoleTemplateId}, SubjectUserPersonaId: {SubjectUserPersonaId}",
                _jobName, batch.EnterpriseRoleBatchProcessId, batch.EnterpriseRoleTemplateId, batch.SubjectUserPersonaId);
            ...
            var response = await apiClient.ProcessEnterpriseRoleBatchAsync(batch, endpoint, cancellationToken);

            logger.LogInformation("{JobName}: Batch {BatchId} processed successfully: {Response}",
                _jobName, batch.EnterpriseRoleBatchProcessId, response);
        }
        catch (Exception ex)
        {
