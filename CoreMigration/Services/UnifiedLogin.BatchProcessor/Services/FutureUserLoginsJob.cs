...
/// <summary>
/// Recurring background service for processing future user logins and sending notifications.
/// Migrated from UserNotification.SendRegularUserNotification.
/// Runs every 15 minutes (configurable).
/// </summary>
public class FutureUserLoginsJob(IServiceProvider serviceProvider, IOptions<BatchProcessorSettings> settings, ILogger<FutureUserLoginsJob> logger, IConfiguration configuration) : BackgroundService
{
    ...
    /// <summary>
    /// Processes future user logins by fetching from database and calling API endpoint.
    /// </summary>
    private async Task ProcessFutureUserLoginsAsync(UserNotificationJobSettings jobSettings, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var usersList = await batchRepository.GetFutureUsersToProcessAsync(jobSettings.BatchSize, cancellationToken);
        ...
        {
            logger.LogError(ex, "{JobName}: Error processing future user logins", _jobName);
            throw;
        }
    }
