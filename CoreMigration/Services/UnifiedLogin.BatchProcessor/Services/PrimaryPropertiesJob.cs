...
/// <summary>
/// Background service for processing primary properties records with improved parallel processing.
///
/// Improvements:
...
/// </summary>
public class PrimaryPropertiesJob(
    ...
    IFeatureFlagService featureFlagService) : BackgroundService
{
    ...
    /// <summary>
    /// Processes primary properties batch records with improved error handling and performance.
    /// </summary>
    private async Task ProcessPendingBatchesAsync(JobSettings jobSettings, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            IList<PrimaryPropertyBatch> pendingBatches;
            ...
            using (var fetchScope = serviceProvider.CreateScope())
            {
                ...
                pendingBatches = await batchRepository.GetPrimaryPropertyProductUpdateBatchToProcessAsync(
                ...
                configLookup = configs
                    .Where(c => c.ConfigurationTypeName == "PrimaryPropertiesBulkUpdateApiEndpoint")
    ...
    /// <summary>
    /// Processes a single primary properties batch record with rate limiting and resilience.
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            logger.LogInformation("{JobName}: Processing batch {BatchId}, BatchProcessTypeId: {BatchProcessTypeId}, SubjectUserPersonaId: {SubjectUserPersonaId}",
                _jobName, batch.PrimaryPropertyBatchProcessId, batch.BatchProcessTypeId, batch.SubjectUserPersonaId);
            ...
            var response = await apiClient.ProcessPrimaryPropertyBatchAsync(batch, endpoint, cancellationToken);

            logger.LogInformation("{JobName}: Batch {BatchId} processed successfully: {Response}",
                _jobName, batch.PrimaryPropertyBatchProcessId, response);
        }
        catch (Exception ex)
        {
