	...
	public class ShellController : ControllerBase
	{
		...
		private List<string> MergeImpersonatorRights(List<string> filterRights, IList<ProductUI> existingProducts)
		{
			...
			var impersonatedUser = impersonatorPersonaList
				.FirstOrDefault(x => x.Organization?.RealPageId.Equals(DefaultUserClaim.EmployeeCompanyRealPageId) == true);
		...
		{
			return _userClaimsAccessor.OrganizationRealPageGuid.Equals(DefaultUserClaim.EmployeeCompanyRealPageId);
		}
