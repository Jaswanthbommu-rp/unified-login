...
public sealed class ManageProductOneSiteAsync : IManageProductOneSiteAsync
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (string.IsNullOrEmpty(systemIdentifier))
        {
            onesitePin = Random.Shared.Next(1, 10000).ToString("D4");
        }
        else
        {
    ...
    private async Task<RightList> GetOneSiteRightsMainAsync(
        Dictionary<string, string> args,
        RequestParameter? datafilter,
        string systemIdentifier,
        CancellationToken ct)
    {
        ...
        {
            FilterSortParameters wsParams = OneSiteHelpers.GenerateSearchAndPaging(
                datafilter, "RightDescription", 0, 9999);
            return await _service.GetRightsListAsync(
                ToNameValuePairs(args), systemIdentifier, wsParams);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "GetOneSiteRightsMainAsync - error");
    ...
    /// <inheritdoc/>
    public async Task<OneSiteUser?> GetOneSiteUserInfoAsync(
    ...
    /// <inheritdoc/>
    public async Task<bool> UserInLeasingAgentListAsync(
        string systemIdentifier, int siteId, CancellationToken ct = default)
    {
        try
        {
            _logger.LogDebug("UserInLeasingAgentListAsync systemIdentifier={SystemIdentifier} siteId={SiteId}",
                systemIdentifier, siteId);
            return await Task.Run(() => _service.GetUserInLeasingAgentList(systemIdentifier, siteId), ct);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex,
                "UserInLeasingAgentListAsync - failed systemIdentifier={SystemIdentifier} siteId={SiteId}",
                systemIdentifier, siteId);
            return false;
        }
    }

    // ── GetUsersForPropertyAsync ───────────────────────────────────────────────

    /// <inheritdoc/>
    public async Task<ListResponse> GetUsersForPropertyAsync(
        ...
        RequestParameter datafilter, CancellationToken ct = default)
    {
        ...
        {
            FilterSortParameters wsParams = OneSiteHelpers.GenerateSearchAndPaging(
                datafilter, "UserName", 0, 9999);
            userList = await _service.GetUsersForPropertyAsync(
                ctx!.EditorProductUserId, propertyId, assignedOnly, wsParams);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "GetUsersForPropertyAsync - error editorPersonaId={EditorPersonaId}", editorPersonaId);
        }

        IList<ProductUser> list = userList.ToGBUsers() ?? [];
        return new ListResponse
        {
            Records     = list.Cast<object>().ToList(),
            TotalRows   = userList.TotalUsers,
            RowsPerPage = 9999,
            ErrorReason = string.Empty,
            TotalPages  = 1
        };
    ...
    // ── GetUsersForRoleAsync ───────────────────────────────────────────────────

    /// <inheritdoc/>
    public async Task<ListResponse> GetUsersForRoleAsync(
        ...
        RequestParameter datafilter, CancellationToken ct = default)
    {
        ...
        try
        {
            FilterSortParameters wsParams = OneSiteHelpers.GenerateSearchAndPaging(
                datafilter, "UserName", 0, 9999);
            userList = await _service.GetUsersForRoleAsync(
                ctx!.EditorProductUserId, roleId, assignedOnly, wsParams);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "GetUsersForRoleAsync - error editorPersonaId={EditorPersonaId}", editorPersonaId);
        }
    ...
    // ── GetRightsCentersAsync ──────────────────────────────────────────────────

    /// <inheritdoc/>
    public async Task<ListResponse> GetRightsCentersAsync(
        long editorPersonaId, CancellationToken ct = default)
    {
        ...
        RightCenter rightCenter = new();
        try
        {
            rightCenter = await _service.GetRightsCentersListAsync(ToNameValuePairs(args), ctx!.EditorProductUserId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "GetRightsCentersAsync - error editorPersonaId={EditorPersonaId}", editorPersonaId);
        }

        rightCenter ??= new RightCenter();
        return new ListResponse
        {
            Records     = rightCenter.RightCenters?.Cast<object>().ToList() ?? [],
            TotalRows   = rightCenter.RightCenters?.Length ?? 0,
            RowsPerPage = 9999,
            ErrorReason = string.Empty,
            TotalPages  = 1
        };
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            RightList rightList = await GetOneSiteRightsMainAsync(args, datafilter, ctx!.EditorProductUserId, ct);
        ...
        {
            _logger.LogError(ex, "GetRightsAsync - error editorPersonaId={EditorPersonaId}", editorPersonaId);
            return new ListResponse { IsError = true, ErrorReason = CommonMessageConstants.RightErrorMessage };
        }
    }
        ...
        RequestParameter datafilter, CancellationToken ct = default)
    {
        ...
        RoleList roleList = new();
        try
        {
            FilterSortParameters wsParams = OneSiteHelpers.GenerateSearchAndPaging(
                datafilter, "RightDescription", 0, 9999);
            roleList = await _service.GetRolesForRightAsync(
                ToNameValuePairs(args), rightId, assignedOnly, wsParams);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "GetRolesForRightAsync - error editorPersonaId={EditorPersonaId}", editorPersonaId);
        }
    ...
    // ── UpdateRightToRolesAsync ────────────────────────────────────────────────

    /// <inheritdoc/>
    public async Task<string> UpdateRightToRolesAsync(
        ...
        bool assignRight, CancellationToken ct = default)
    {
        ...
        AssignStatus status = new();
        try
        {
            status = await _service.ModifyRightToRolesAsync(
                ctx!.EditorProductUserId, rightId, roleIdList, assignRight);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "UpdateRightToRolesAsync - error editorPersonaId={EditorPersonaId}", editorPersonaId);
            status.ErrorMessage = ParseSoapErrorMessage(ex.Message);
        }
        return status.ErrorMessage ?? string.Empty;
    }

    // ── UpdateRoleToRightsAsync ────────────────────────────────────────────────

    /// <inheritdoc/>
    public async Task<string> UpdateRoleToRightsAsync(
        ...
        CancellationToken ct = default)
    {
        ...
        string rightsToAddList    = (rightsToAdd    is { Count: > 0 }) ? string.Join("|", rightsToAdd)    : string.Empty;
        string rightsToRemoveList = (rightsToRemove is { Count: > 0 }) ? string.Join("|", rightsToRemove) : string.Empty;
        ...
        try
        {
            ...
            {
                status = await _service.ModifyRoleToRightsAsync(
                    ctx!.EditorProductUserId, roleId, rightsToAddList, rightsToRemoveList);
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "UpdateRoleToRightsAsync - error editorPersonaId={EditorPersonaId}", editorPersonaId);
            status.ErrorMessage = ParseSoapErrorMessage(ex.Message);
        }
        return status.ErrorMessage ?? string.Empty;
    }
        ...
        CancellationToken ct = default)
    {
        ...
        string      roleToAlter = roleId == 0 ? string.Empty : roleId.ToString();
        RoleList    roleList    = new();
        AssignStatus status     = new();
        try
        {
            roleList = await _service.AddUpdateRoleAsync(
                ctx!.EditorProductUserId, roleToAlter, roleName, inheritRoleId);
            status.ErrorMessage = string.Empty;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "AddUpdateRoleAsync - error editorPersonaId={EditorPersonaId}", editorPersonaId);
            status.ErrorMessage = ParseSoapErrorMessage(ex.Message);
        }

        IList<ProductRole> list = roleList.TotalRoles != 0 ? roleList.ToGBRoles() ?? [] : [];
        return new ListResponse
        {
            Records     = list.Cast<object>().ToList(),
            TotalRows   = list.Count,
            RowsPerPage = 9999,
            ErrorReason = status.ErrorMessage ?? string.Empty,
            TotalPages  = 1
        };
    }
        ...
        long editorPersonaId, int roleId, CancellationToken ct = default)
    {
        ...
        AssignStatus status = new() { ErrorMessage = string.Empty };
        try
        {
            await _service.DeleteRoleAsync(ctx!.EditorProductUserId, roleId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "DeleteRoleAsync - error editorPersonaId={EditorPersonaId}", editorPersonaId);
            status.ErrorMessage = ParseSoapErrorMessage(ex.Message);
        }
        return status.ErrorMessage ?? string.Empty;
    }
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            _logger.LogDebug("UnassignUserAsync - DisableUserAsync systemIdentifier={SystemIdentifier}", ctx!.ProductUserId);
            await _service.DisableUserAsync(ctx!.ProductUserId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "UnassignUserAsync - error editorPersonaId={EditorPersonaId} userPersonaId={UserPersonaId}",
                editorPersonaId, userPersonaId);
            return ex.Message;
        }
        ...
        {
            await _samlRepository.DeleteSamlUserProductInfoAndStatusAsync(userPersonaId, ProductId, ct);
        }
    ...
    /// <inheritdoc/>
    public async Task<(string resultCount, List<AdditionalParameters> auditParams)> UpdateRolesForUserAsync(
        long editorPersonaId, long userPersonaId,
        List<string> rolesToAssign, CancellationToken ct = default)
    {
        ...
        bool   isSuperUser = await IsSuperUserAsync(ctx!.UserPersona!, ct);
        string pmcId       = await GetOneSitePmcIdFromPersonaAsync(ctx.EditorPersona, ct);

        return await UpdateRolesForUserInternalAsync(
            ctx.ProductUserId, pmcId, isSuperUser, rolesToAssign, ct);
    }

    // ── UpdatePropertiesForUserAsync ───────────────────────────────────────────

    /// <inheritdoc/>
    public async Task<(string resultCount, List<AdditionalParameters> auditParams)> UpdatePropertiesForUserAsync(
        long editorPersonaId, long userPersonaId,
        List<string> propertiesToAssign, CancellationToken ct = default)
    {
        ...
        return await UpdatePropertiesForUserInternalAsync(
            ctx.ProductUserId, pmcId, propertiesToAssign, ct);
    }
    ...
    public async Task<PMCInfo?> GetPmcInfoAsync(int pmcId, CancellationToken ct)
    {
    ...
    /// <summary>Extracts the actual message from a SOAP fault envelope.</summary>
    private static string ParseSoapErrorMessage(string message)
    {
        const string open  = "<ErrorMessage>";
        const string close = "</ErrorMessage>";
        int start = message.IndexOf(open,  StringComparison.OrdinalIgnoreCase);
        int end   = message.IndexOf(close, StringComparison.OrdinalIgnoreCase);
        return (start >= 0 && end > start)
            ? message[(start + open.Length)..end]
            : message;
    }
}
