...
public sealed class ManageProductOpsAsync : IManageProductOpsAsync
{
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (assetGroupId == 0)
            {
                assetGroups = JsonConvert.DeserializeObject<List<AssetGroup>>(body) ?? [];
            }
            else
            {
        ...
        CancellationToken ct = default)
    {
        ...
        IList<AssetGroup> assetGroupList = [];
        try
        {
            int moduleAssetGroups = await GetModuleAssetGroupsConfigAsync(session, ct);
            if (moduleAssetGroups != 1)
        ...
        CancellationToken ct = default)
    {
        ...
        IList<AssetGroupPatch> patchList = [assetGroup];
        try
        {
            int moduleAssetGroups = await GetModuleAssetGroupsConfigAsync(session, ct);
            if (moduleAssetGroups != 1)
        ...
        CancellationToken ct = default)
    {
        ...
        List<string> rightsToAdd    = [];
        List<string> rightsToRemove = [];
        List<object> rightsInput    = [];
        ...
        if (roleId == 0)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    roleListResponse = new ListResponse { Records = (new[] { result }).Cast<object>().ToList(), TotalRows = 1, RowsPerPage = 1, TotalPages = 1, ErrorReason = "", IsError = false };
        ...
        CancellationToken ct = default)
    {
        ...
        bool isSuperUser = await _contextService.IsSuperUserAsync(userPersona, ct);
        ...
        if (string.IsNullOrEmpty(ctx.ProductUserId))
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    using var createDoc = JsonDocument.Parse(await response.Content.ReadAsStringAsync(ct));
                    string newid     = createDoc.RootElement.TryGetProperty("id",         out var idEl) ? idEl.GetString()  ?? string.Empty : string.Empty;
                    string loginName = createDoc.RootElement.TryGetProperty("login_name", out var lnEl) ? lnEl.GetString() ?? string.Empty : string.Empty;
        ...
        else
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    using var updateDoc = JsonDocument.Parse(await response.Content.ReadAsStringAsync(ct));
                    string newid = updateDoc.RootElement.TryGetProperty("id", out var idEl2) ? idEl2.GetString() ?? string.Empty : string.Empty;
                    var samlAttr = new SamlAttributes { SamlAttributeId = (int)SamlAttributeEnum.UserId, Value = newid };
        ...
        CancellationToken ct = default)
    {
        ...
        req.Content = new StringContent(JsonConvert.SerializeObject(opsMigrateUsers), Encoding.UTF8, "application/json");
        ...
        string baseUrl, CancellationToken ct)
    {
        ...
        {
            byte[] hash = md5.ComputeHash(Encoding.UTF8.GetBytes(loginName + key));
        ...
        for (int attempt = 0; attempt < MaxRetryCount; attempt++)
        {
            try
            {
                ...
                {
                    Content = new StringContent(JsonConvert.SerializeObject(sessionRequest), Encoding.UTF8, "application/json")
                };
                using var response = await client.SendAsync(req, ct);

                if (response.IsSuccessStatusCode)
                {
                    using var sessionDoc = JsonDocument.Parse(await response.Content.ReadAsStringAsync(ct));
                    string? sid = sessionDoc.RootElement.TryGetProperty("session", out var sessionEl)
                               && sessionEl.TryGetProperty("sid", out var sidEl)
    ...
    private async Task<int> GetModuleAssetGroupsConfigAsync(OpsSession session, CancellationToken ct)
    {
        ...
        using var configDoc = JsonDocument.Parse(await response.Content.ReadAsStringAsync(ct));
        return configDoc.RootElement.TryGetProperty("MODULE_ASSET_GROUPS", out var moduleEl)
            ? moduleEl.GetInt16() : (short)0;
    }
    ...
    private async Task<OpsUser?> OpsGetUserByIdAsync(OpsSession session, string productUserId, CancellationToken ct)
    {
        ...
        using var userDoc = JsonDocument.Parse(await response.Content.ReadAsStringAsync(ct));
        var r = userDoc.RootElement;

        string? GetProp(JsonElement el, string name) =>
            el.TryGetProperty(name, out var v) ? v.GetString() : null;

        return new OpsUser
        {
            ID         = GetProp(r, "id"),
            FirstName  = GetProp(r, "first_name"),
            MiddleName = GetProp(r, "middle_name") ?? string.Empty,
            LastName   = GetProp(r, "last_name"),
            Loginname  = GetProp(r, "login_name"),
            AssetID    = r.TryGetProperty("asset",     out var assetEl) ? GetProp(assetEl,  "id") ?? string.Empty : string.Empty,
            UserTypeId = r.TryGetProperty("user_type", out var utEl)    ? GetProp(utEl,     "id") : null,
    ...
    private async Task<bool> OpsLoginNameInUseAsync(OpsSession session, string loginName, CancellationToken ct)
    {
        ...
        using var loginDoc = JsonDocument.Parse(await response.Content.ReadAsStringAsync(ct));
        string? foundLogin = loginDoc.RootElement.TryGetProperty("login_name", out var lnEl2) ? lnEl2.GetString() : null;
    ...
    private async Task<ListResponse> GetAllRolesAsync(OpsSession session, CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                IList<Role> rolesList = JsonConvert.DeserializeObject<IList<Role>>(await httpResp.Content.ReadAsStringAsync(ct)) ?? [];
        ...
        long userPersonaId, string? assetCode, CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                IList<Role> rolesList = JsonConvert.DeserializeObject<IList<Role>>(await httpResp.Content.ReadAsStringAsync(ct)) ?? [];
                IList<ProductRole> productRoles = rolesList.ToGBRoles() ?? [];
        ...
        CancellationToken ct)
    {
        try
        {
            ...
            if (moduleAssetGroups == 1)
            {
                ...
                var assetGroups = JsonConvert.DeserializeObject<List<AssetGroup>>(await response.Content.ReadAsStringAsync(ct)) ?? [];
            ...
            else
            {
                ...
                var portfolioList = JsonConvert.DeserializeObject<List<Portfolio>>(await response.Content.ReadAsStringAsync(ct)) ?? [];
