		...
		private void RunPendingProcess()
		{
			...
			while (!cancellation.WaitHandle.WaitOne(interval))
			{
				try
				{
					...
					Logger.Write(LogType.Diagnostic, new LogDetails { Message = "RunPendingProcess - All threads processed." });

					// Occasionally check the cancellation state.
					if (cancellation.IsCancellationRequested)
					{
						Logger.Write(LogType.Information, new LogDetails { Message = "RunPendingProcess - Thread cancellation requested." });
						break;
					}
					interval = _waitAfterSuccessInterval;
				}
				...
				{
					// Log the exception. 
					Logger.Write(LogType.Error, new LogDetails { Message = "RunPendingProcess - Exception in main task.", Exception = ex });
