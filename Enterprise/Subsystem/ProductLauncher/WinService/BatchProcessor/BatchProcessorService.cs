...
namespace RP.Enterprise.Subsystem.ProductLauncher.WinService.UnityBatchProcessor
{
    public partial class BatchProcessorService : ServiceBase
    {
        ...
        protected override void OnStart(string[] args)
        {
            try
            {
                Log.Information("{methodName} - {state}", propertyValue: new object[] { "OnStart", $"Batch Processor Windows Service Starting... Database used -{_dbServerName}" });
                ...
                Log.Information("{methodName} - {state}", propertyValue: new object[] { "OnStart", "Launched retry polling task..." });
                ...
                Log.Information("{methodName} - {state}", propertyValue: new object[] { "OnStart", "Launched Enterprise Role Update task..." });
                ...
                Log.Information("{methodName} - {state}", propertyValue: new object[] { "OnStart", "Launched Primary Properties Update task..." });
                ...
                Log.Information("{methodName} - {state}", propertyValue: new object[] { "OnStart", "Launched enterprise role product update polling task..." });
#if (DEBUG)
                Console.WriteLine("-------------------------------------------------------------------------------");
#endif
            }
        ...
        {
            Log.Information("{methodName} - {state}", propertyValue: new object[] { "OnStop", "Batch Processor Windows Service Stopping..." });
            _cts.Cancel();
        }
        ...
        {
            Log.Information("{methodName} - {state}", propertyValue: new object[] { "RunRetryProcess", $"Starting in polling main task :threadCount - {ThreadCount}, batchSize - {BatchSize}, pollingInterval - {RetryPollingInterval}" });
            ...
            while (!cancellation.WaitHandle.WaitOne(interval))
            {
                try
                {
                    Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunRetryProcess", "Getting product batch to retry process." });
                    ...
                    {
                        Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunRetryProcess", "No items to process in the batch." });
                        interval = _waitForRetryInterval;
                        continue;
                    }

                    Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunRetryProcess", $"Launching threads to process {batch.Count} products." });
                    ...
                    Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunRetryProcess", "All threads processed." });

                    // Occasionally check the cancellation state.
                    if (cancellation.IsCancellationRequested)
                    {
                        Log.Information("{methodName} - {state}", propertyValue: new object[] { "RunRetryProcess", "Thread cancellation requested." });
                        break;
                    }
                ...
                {
                    // Log the exception.
                    Log.Error(ex, "{methodName} - {state}", propertyValue: new object[] { "RunRetryProcess", "RunRetryProcess - Exception in main task." });
        ...
        {
            Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunPendingProcess", $"Starting in polling main task :threadCount - {ThreadCount}, batchSize - {BatchSize}, pollingInterval - {PollingInterval}" });
#if (DEBUG)
            Console.WriteLine("-------------------------------------------------------------------------------");
#endif
            CancellationToken cancellation = _cts.Token;
            ...
            while (!cancellation.WaitHandle.WaitOne(interval))
            {
                try
                {
                    Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunPendingProcess", "Getting product batch to process." });
                    ...
                    if (batch == null || batch.Count <= 0)
                    {
                        Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunPendingProcess", "No items to process in the batch." });
                        interval = _waitAfterSuccessInterval;
                        continue;
                    }

                    Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunPendingProcess", $"Launching threads to process {batch.Count} products." });

                    ThreadCount = GetProductInternalSettings("BatchProcessorPendingThread");
                    // Launch threads
                    Parallel.ForEach(batch, new ParallelOptions { MaxDegreeOfParallelism = ThreadCount }, CallApiToProcessBatchRecord);

                    Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunPendingProcess", "All threads processed." });

                    // Occasionally check the cancellation state.
                    if (cancellation.IsCancellationRequested)
                    {
                        Log.Information("{methodName} - {state}", propertyValue: new object[] { "RunPendingProcess", "Thread cancellation requested." });
                        break;
                    }
                    interval = _waitAfterSuccessInterval;
                }
                ...
                {
                    // Log the exception. 
                    Log.Error(ex, "{methodName} - {state}", propertyValue: new object[] { "RunPendingProcess", "Exception in main task." });
        ...
        {
            Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunEnterpriseRoleUpdateProcess", $"Starting in polling main task :threadCount - {ThreadCount}, batchSize - {BatchSize}, pollingInterval - {PollingInterval}" });
#if (DEBUG)
            Console.WriteLine("-------------------------------------------------------------------------------");
#endif
            CancellationToken cancellation = _cts.Token;
            ...
            while (!cancellation.WaitHandle.WaitOne(interval))
            {
                try
                {
                    Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunEnterpriseRoleUpdateProcess", "Getting product batch to process." });
                    ...
                    if (batch == null || batch.Count <= 0)
                    {
                        Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunEnterpriseRoleUpdateProcess", "No items to process in the batch." });
                        interval = _waitAfterSuccessInterval;
                        continue;
                    }

                    Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunEnterpriseRoleUpdateProcess", $"Launching threads to process {batch.Count} products." });
                    ...
                    Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunEnterpriseRoleUpdateProcess", "All threads processed." });

                    // Occasionally check the cancellation state.
                    if (cancellation.IsCancellationRequested)
                    {
                        Log.Information("{methodName} - {state}", propertyValue: new object[] { "RunEnterpriseRoleUpdateProcess", "Thread cancellation requested." });
                        break;
                    }
                    interval = _waitAfterSuccessInterval;
                }
                ...
                {
                    // Log the exception. 
                    Log.Error(ex, "{methodName} - {state}", propertyValue: new object[] { "RunEnterpriseRoleUpdateProcess", "Exception in main task." });
        ...
        {
            Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunPrimaryPropertiesUpdateProcess", $"Starting in polling main task :threadCount - {ThreadCount}, batchSize - {BatchSize}, pollingInterval - {PollingInterval}" });
            #if (DEBUG)
            Console.WriteLine("-------------------------------------------------------------------------------");
            #endif
            CancellationToken cancellation = _cts.Token;
            ...
            while (!cancellation.WaitHandle.WaitOne(interval))
            {
                try
                {
                    Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunPrimaryPropertiesUpdateProcess", "Getting product batch to process." });
                    ...
                    if (batch == null || batch.Count <= 0)
                    {
                        Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunPrimaryPropertiesUpdateProcess", "No items to process in the batch." });
                        interval = _waitAfterSuccessInterval;
                        continue;
                    }

                    Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunPrimaryPropertiesUpdateProcess", $"Launching threads to process {batch.Count} products." });
                    ...
                    Log.Debug("{methodName} - {state}", propertyValue: new object[] { "RunPrimaryPropertiesUpdateProcess", "All threads processed." });

                    // Occasionally check the cancellation state.
                    if (cancellation.IsCancellationRequested)
                    {
                        Log.Information("{methodName} - {state}", propertyValue: new object[] { "RunPrimaryPropertiesUpdateProcess", "Thread cancellation requested." });
                        break;
                    }
                    interval = _waitAfterSuccessInterval;
                }
                ...
                {
                    // Log the exception. 
                    Log.Error(ex, "{methodName} - {state}", propertyValue: new object[] { "RunPrimaryPropertiesUpdateProcess", "Exception in main task." });
        ...
        private void CallApiToProcessBatchRecord(Batch batch)
        {
            ...
            try
            {
                ...
                logger.Debug("{methodName} - {state}", propertyValue: new object[] { "CallApiToProcessBatchRecord", $"Working to assign product {batch.ProductId} to user {batch.SubjectUserPersonaId}." });
                ...
                logger.Information("{methodName} - {state}", propertyValue: new object[] { "CallApiToProcessBatchRecord", $"Result received for Product {input.ProductId} & for User {batch.SubjectUserPersonaId} - {result.Result}. Calling API Completed to assign product {input.ProductId} to user {batch.SubjectUserPersonaId}." });
            }
            catch (Exception ex)
            {
                // Log the exception. 
                ...
                logger = logger.ForContext("ProductModule", this.GetType());
                logger = logger.ForContext("CorrelationId", batch.CorrelationId);
                logger.Error(ex, "{methodName} - {state}", propertyValue: new object[] { "CallApiToProcessBatchRecord", $"Exception while calling API for ProductId {batch.ProductId}." });
        ...
        private void CallApiToProcessEnterpriseRoleBatchRecord(EnterpriseRoleBatch batch)
        {
            ...
            try
            {
                ...
                logger = logger.ForContext($"AdditionalInfo", JsonConvert.SerializeObject(additionalInfo, Formatting.Indented), true);
                logger = logger.ForContext("ProductModule", this.GetType());
                logger.Debug("{methodName} - {state}", propertyValue: new object[] { "CallApiToProcessEnterpriseRoleBatchRecord", $"Working to assign products to user from enterprise role template {batch.EnterpriseRoleTemplateId} to user {batch.SubjectUserPersonaId}." });

                var landingApiCaller = new ProductApiCaller();
                var result = landingApiCaller.ProcessEnterpriseRoleBatchRecord(batch, processEndpoint);

                logger.Information("{methodName} - {state}", propertyValue: new object[] { "CallApiToProcessEnterpriseRoleBatchRecord", $"Result received for User {batch.SubjectUserPersonaId} - {result.Result}." });
            }
            catch (Exception ex)
            {
                Exception realError = ex;
                ...
                logger.Error(ex, "{methodName} - {state}", propertyValue: new object[] { "CallApiToProcessEnterpriseRoleBatchRecord", $"Exception while CallApiToProcessEnterpriseRoleBatchRecord {batch.EnterpriseRoleTemplateId}." });
        ...
        private void CallApiToProcessPrimaryPropertiesBatchRecord(PrimaryPropertyBatch batch)
        {
            ...
            try
            {
                ...
                logger = logger.ForContext($"AdditionalInfo", JsonConvert.SerializeObject(additionalInfo, Formatting.Indented), true);
                logger = logger.ForContext("ProductModule", this.GetType());
                logger.Debug("{methodName} - {state}", propertyValue: new object[] { "CallApiToProcessPrimaryPropertiesBatchRecord", $"Working to update products to user with primary properties {batch.PrimaryPropertyBatchProcessId} to user {batch.SubjectUserPersonaId}." });

                var landingApiCaller = new ProductApiCaller();
                var result = landingApiCaller.ProcessPrimaryPropertyBatchRecord(batch, processEndpoint);

                logger.Information("{methodName} - {state}", propertyValue: new object[] { "CallApiToProcessPrimaryPropertiesBatchRecord", $"Result received for User {batch.SubjectUserPersonaId} - {result.Result}." });
            }
            catch (Exception ex)
            {
                Exception realError = ex;
                ...
                logger = logger.ForContext("ProductModule", this.GetType());
                logger = logger.ForContext("InnerException", realError);
                logger.Error(ex, "{methodName} - {state}", propertyValue: new object[] { "CallApiToProcessPrimaryPropertiesBatchRecord", $"Exception while {batch.PrimaryPropertyBatchProcessId}." });
        ...
        private string GetBatchConfigurationByType(Guid correlationId, int batchBatchProcessTypeId, string configurationTypeName)
        {
            ...
            logger.Debug("{methodName} - {state}", propertyValue: new object[] { "GetBatchConfigurationByType", $"Get information for batchBatchProcessTypeId {batchBatchProcessTypeId}" });
            ...
            logger.Debug("{methodName} - {state}", propertyValue: new object[] { "GetBatchConfigurationByType", $"Endpoint received for {batchBatchProcessTypeId} - {endpoint}" });
