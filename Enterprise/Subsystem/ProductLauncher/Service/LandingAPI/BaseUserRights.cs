...
using RP.Enterprise.Subsystem.ProductLauncher.Service.LandingAPI.Logic;
using RP.Enterprise.Subsystem.ProductLauncher.Service.LandingAPI.Repository;
using System;
	...
	public static class BaseUserRights
	{
		...
		public static List<string> GetUserRightsBy(ClaimsPrincipal userPrincipal, DefaultUserClaim userClaim)
		{
			...
			if (userClaim.ImpersonatedBy != Guid.Empty)
			{
				...
				Persona impersonateUserPersona = mp.GetActivePersona(userClaim.ImpersonatedBy);
		...
		/// </summary>
		/// <param name="claims">The current claims of the user being logged in</param>
		...
		{
			if (!identity.Claims.Any(q => q.Type.ToUpper().Equals("RIGHT") && q.Value.ToUpper().Equals(rightName)))
			{
				// the user being logged into doesn't have the right, so see if the impersonator does
				if (roleRights.Any(p => p.ToUpper().Equals(rightName.ToUpper())))
				{
					string right = roleRights.FirstOrDefault(p => p.ToUpper().Equals(rightName.ToUpper()));
		...
		private static IList<UserRoleRights> GetCompanyRoles(DefaultUserClaim userClaim, long orgPartyId, Guid orgGuid)
		{
			...
			var cacheKey = $"getRolesByParty_{orgPartyId}_{(int) ProductEnum.UnifiedLogin}";
			IList<UserRoleRights> roleList = rpCache.GetFromCache<IList<UserRoleRights>>(cacheKey, 180, () =>
			{
                ...
                ProductRepository pr = new ProductRepository(userClaim);
				IList<int> productList = pr.GetProductIdsByCompany(orgGuid);
		...
		private static List<string> GetRights(IList<UserRoleRights> companyRoles, long roleId, long personaId, long orgPartyId)
		{
			RPObjectCache rpCache = new RPObjectCache();
			// get user rights
			string cacheKey = $"getRightsByPersona_{orgPartyId}_{personaId}";

			List<string> userRights = rpCache.GetFromCache<List<string>>(cacheKey, 30, () =>
			{
				...
				if (companyRoles.Any(r => r.RoleId == roleId))
				{
					...
					{
						if (!string.IsNullOrWhiteSpace(right.RightNickName) && !string.IsNullOrWhiteSpace(right.RightNickName.Trim()) && !rights.Contains(right.RightNickName))
						{
							rights.Add(right.RightNickName);
						}
					}
				}
