...
namespace RP.Enterprise.Subsystem.ProductLauncher.LandingAPI.Test.Logic
{
	...
	public class RouteEnterpriseTestBase
	{
		...
		public RouteEnterpriseTestBase(HttpConfiguration Config, DefaultHttpControllerSelector ControllerSelector)
		{
			_config = Config;
        ...
        /// <returns></returns>
        public string VerifyEnterpriseRouteToAction(HttpMethod method, string url)
        {
            try
            {
                // remove the leading /api as the services are now running under their own application
                url = url.Replace("/apienterprise", "");
