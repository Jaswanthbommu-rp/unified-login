 ...
 SET NOCOUNT ON;  
          ...
          drop table if exists #TempUPFMProperties  
                ...
                set @selectedcolName = (select ProductName from #DistinctColumns where ID = @rcnt)  
                      ...
                      set @insertsqlstmt = @insertsqlstmt + ' select PropertyName from #TempUPFMProperties where PropertyGUID in (select distinct  UPFMProductGUID from #TempFinal where ' + @selectedcolName + ' is null )'  
                    exec (@insertsqlstmt)      
                   set @rcnt = @rcnt + 1 ;  
