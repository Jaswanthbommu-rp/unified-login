 ...
 */    
 ...
 --Insert Roles From Json to TempTable    
 Declare @RoleTemplateProductAdditionTab nvarchar(max) = null
 SELECT @RoleTemplateProductAdditionTab = RoleTemplateProductBatch
 FROM      
  OPENJSON (@json)      
  WITH (    
   RoleTemplateProductBatch nvarchar(max) as json    
   )    
   As RoleTemplateProductBatch   
    

 IF (ISNULL(@RoleTemplateProductAdditionTab ,'')  <> '')
 BEGIN
 IF NOT EXISTS (SELECT TOP 1 1 FROM [Security].[RoleTemplateProductAdditionalTab] WHERE [RoleTemplateId] = @RoleTemplateId)
 BEGIN
 INSERT INTO [Security].[RoleTemplateProductAdditionalTab] ([RoleTemplateId],[TabJson],[CreatedDate],[ModifiedDate])
 VALUES(@RoleTemplateId,@RoleTemplateProductAdditionTab,GETUTCDATE(),GETUTCDATE())
 END
 ELSE
 BEGIN
 UPDATE [Security].[RoleTemplateProductAdditionalTab] SET [TabJson] = @RoleTemplateProductAdditionTab,[ModifiedDate] = GETUTCDATE() WHERE [RoleTemplateId] = @RoleTemplateId
 END
 END
