...
-- =============================================
CREATE PROCEDURE [UserManagement].SearchTabDependency (
...
BEGIN
	...
	AND
		(@ControlIdentifier IS NULL OR [ControlIdentifier] = @ControlIdentifier OR CHARINDEX(@ControlIdentifier,[ControlIdentifier]) > 0)
	AND
		(@TabTypeId IS NULL  OR  [TabTypeId] = @TabTypeId)
	AND
		(@ControlValue IS NULL OR [ControlValue] = @ControlValue OR CHARINDEX(@ControlValue,[ControlValue]) > 0)
	AND
