...
begin                  
 ...
 select 0              
  ...
  if(isnull(@Sequence,0)<1 and isnull(@ParentControlId,0)>0)      
  select @Sequence=(count(*)+1) from [UserManagement].[Control] where [ParentControlId]=@ParentControlId      
 ...
 BEGIN CATCH               
  print ERROR_MESSAGE()                
        ...
        SELECT  0 AS Id,                    
   ErrorMessage                    
        FROM    dbo.ErrorLog                          WHERE   ErrorLogID = @ErrorLogID;                    
 END CATCH                    
END;