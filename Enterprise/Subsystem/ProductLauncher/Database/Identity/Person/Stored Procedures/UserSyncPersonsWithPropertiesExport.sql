CREATE PROCEDURE [Person].[UserSyncPersonsWithPropertiesExport] (           
 @ProperyOperators [Person].[BookPropertyOperatorType] READONLY,
...
BEGIN      
 ...
 UserOperator nvarchar(max) NULL,    
 PropertyOperator nvarchar(max) NULL,    
 PrimaryProperty nvarchar(max) NULL,    
 UPFMSourceId nvarchar(max) NULL,    
 ProductUserName nvarchar(max) NULL,    
 ProductSiteId INT NULL    
  ...
  END;      
   ...
   WITH     
   CTE_PersonaAssignedPrimaryProperties(PersonaId,AssignedCount)       
    AS (      
 ...
 ORDER BY USJ.UserSyncJobId desc    
   ...
   INNER JOIN Enterprise.PersonaProductPropertiesSyncHistory PMH ON      
    PMH.PersonaId = P.PersonaId     
 And PMH.ProductId = @ProductId      
    ),    
 CTE_UserOperator(PersonaId, CompanyName)    
 AS(    
  Select P.PersonaId, o.Name     
  From @ValidPersona P    
  inner join [Enterprise].ExternalUserRelationship r     
   on p.UserLoginPersonaId = r.UserLoginPersonaId    
  INNER JOIN [Enterprise].Organization o ON o.PartyId = r.ThirdPartyCompanyPartyId    
  WHERE r.ThirdPartyRelationshipId = 1    
 ),    
   CTE_ProductUserName(PersonaId, ProductUserName)    
   AS(    
   select p.personaId, s.[value]    
  from @ValidPersona p    
  left outer join [Ident].SamlUserAttribute s on p.PersonaId = s.personaId     
  Where s.productId = @productId    
  and s.SamlAttributeId = 1    
   ),    
  ...
  pe.PersonaId = o.PersonaId    
--   LEFT JOIN CTE_PrimaryProperty pp ON     
--  pe.PersonaId = pp.PersonaId    
   LEFT JOIN CTE_ProductUserName pun ON    
