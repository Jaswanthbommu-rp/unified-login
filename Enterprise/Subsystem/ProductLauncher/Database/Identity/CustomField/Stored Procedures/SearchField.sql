...
-- =============================================
CREATE PROCEDURE [CustomField].[SearchField] (
	 @FieldId BIGINT = NULL 
...
BEGIN
	...
	AND
		(@DefaultValue IS NULL OR [DefaultValue] = @DefaultValue OR CHARINDEX(@DefaultValue,[DefaultValue]) > 0)
	AND
		(@SyncField IS NULL OR [SyncField] = @SyncField OR CHARINDEX(@SyncField,[SyncField]) > 0)
	AND
		(@Sequence IS NULL  OR  [Sequence] = @Sequence)
	AND
		(@HelpText IS NULL OR [HelpText] = @HelpText OR CHARINDEX(@HelpText,[HelpText]) > 0)
	AND
