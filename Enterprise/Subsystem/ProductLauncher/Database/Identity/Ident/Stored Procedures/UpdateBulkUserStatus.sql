				   ...
				   UPDATE ULP
					...
					UPDATE ULP
					 ...
					 FROM #UserList ULi
						...
						INNER JOIN Ident.UserLoginPersona ULP ON ULP.UserLoginId = UL.UserId AND ULP.OrganizationPartyId = @PartyId
					WHERE
						(((UL.LastLoginDate IS NOT NULL) OR (UL.LastLoginDate IS NULL AND UL.PasswordHash IS NOT NULL)) AND UL.IdentityProviderTypeId = @NonIDPTypeID)
						AND ULI.PStatus = 24 -- current user status is disabled
