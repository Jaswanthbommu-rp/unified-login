...
declare @currentSettingType varchar(100)
...
BEGIN
	...
	VALUES( @ProductId, @ProductSettingTypeId, 1, GETUTCDATE() );
END;
...
Select @currentSettingType = 'SystemAdminUserDefaultRole',@currentsettingValue = 'Role-IntMgr',@CurrentProductId = 39

if not exists ( select top 1 1 from Enterprise.ProductSettingType where name = 'SystemAdminUserDefaultRole' )
begin
	insert into enterprise.ProductSettingType ( name, Description ) values ( 'SystemAdminUserDefaultRole', 'System Administrator Product Default Role')
end 
if not exists (
	select top 1 1 
		FROM Enterprise.GlobalProductConfiguration gpc  
...
IF EXISTS(SELECT 1 FROM Enterprise.RightValueType WHERE Value = 'Employee Access to Manage Questionnaires in CIMPL')
BEGIN
	UPDATE Enterprise.RightValueType
		SET Value = 'Employee Access to Design Questionnaires in CIMPL'
	WHERE Value = 'Employee Access to Manage Questionnaires in CIMPL'
END
GO
IF EXISTS(SELECT 1 FROM Enterprise.RightValueType WHERE Value = 'Employee View-Only Access to CIMPL')
BEGIN
	UPDATE Enterprise.RightValueType
		SET Value = 'Employee Access to CIMPL'
	WHERE Value = 'Employee View-Only Access to CIMPL'
END
GO
...
VALUES( 1, 'Employee Access to CIMPL: Standard Implementation Access', 'Employee Access to CIMPL: Standard Implementation Access', 'CIMPLStandardImplementationAccess' );
...
(
	...
	WHERE ObjectValue = 'CIMPL Standard Implementation Access' AND 
		  ParentActionId IS NULL
)
BEGIN
	EXEC Enterprise.CreateAction @ProductID = @ProductId, @Action = N'CIMPL Standard Implementation Access', @ActionTarget = N'Right', @ActionbValueTypeId = 1, @Description = '', @ActionID = @ActionID OUTPUT;
...
BEGIN
	EXEC [Enterprise].[CreateAction] @ProductID = @ProductId, @Action = N'CIMPL Standard Implementation Access', @ActionTarget = N'Right', @ActionbValueTypeId = 1, @Description = '', @ParentActionID = @ParentActionId, @ActionID = @ActionID OUTPUT;
	SELECT @ActionID AS N'@ActionID';
END;
...
BEGIN
	...
	BEGIN
		...
		WHERE RR.Value = 'Default_CIMPLStandardImplementationAccess';
		SELECT @ActionID = ActionID
		FROM Enterprise.ACTION
		WHERE ObjectValue = 'CIMPL Standard Implementation Access'   AND 
			  ObjectType = 'Right' AND 
...
WHERE value = 'Default_CIMPLStandardImplementationAccess';

SELECT @RightValueTypeId = RightValueTypeId
FROM Enterprise.RightValueType
WHERE value IN( 'Employee Access to CIMPL: Standard Implementation Access' );
