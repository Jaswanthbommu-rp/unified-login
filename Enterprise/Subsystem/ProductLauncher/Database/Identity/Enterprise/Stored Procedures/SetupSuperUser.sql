...
BEGIN
	...
	*/
	DECLARE @NOW DATETIME = GETUTCDATE(),
	 ...
	 IF (SELECT COUNT(*) FROM @AssignedProductId) = 0
	BEGIN
		SELECT	0 AS Id,
	...
	IF NOT EXISTS (SELECT 1 FROM Ident.UserLogin WHERE LoginName = @Email)
	BEGIN
		...
		IF @DefaultIDP = 1 AND @IDPTypeId IS NULL
		BEGIN
