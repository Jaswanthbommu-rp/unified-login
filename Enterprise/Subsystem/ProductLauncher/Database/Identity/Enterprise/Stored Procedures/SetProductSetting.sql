...
BEGIN          
 BEGIN TRY            
  BEGIN TRAN;          
   ...
   if(not exists(select top 1 1 from Enterprise.ProductSetting  
    where ThruDate is null and Value = @Value and ProductSettingTypeId=@ProductSettingTypeId  
   ...
   Begin  
    ...
    select top 1 @TempSettingId=ProductSettingId from Enterprise.ProductSetting   
    where ProductSettingTypeId = @ProductSettingTypeId         
     and ProductId = @ProductId        
     and @now between FromDate and Isnull(ThruDate, @nowPlusSec)        
