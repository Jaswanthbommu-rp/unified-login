...
(function(angular, undefined) {
    ...
    function OnesiteNewRoleTabsManager($rootScope, timeout, pubsub, tabsData, tabsMenu, aside, newRoleModel, newRoleSvc, user, persona) {
        var svc = this,
            triggerID = "OnesiteNewRoleTabsManager";
        ...
        svc.init = function() {
            ...
            svc.updateWatch = pubsub.subscribe("newRoleOnesite.update", svc.onUpdate);
            svc.cancelWatch = pubsub.subscribe("newRoleOnesite.cancel", svc.onCancel);
        ...
        svc.onCreateRoleSuccess = function(resp) {
            ...
            if (!angular.isUndefined(resp.errorReason) && resp.errorReason.trim().length === 0 && resp.isError === false) {
                ...
                pubsub.publish("osSettings.newRoleError", resp);
            }
        };

        svc.onCreateRoleError = function(resp) {
            pubsub.publish("osSettings.newRoleError", resp.data);
        };
        ...
        svc.onUpdateSuccess = function() {
            ...
            if (svc.reqCount === svc.reqSuccessCount) {
                aside.hide();
                svc.resetCounts();
                pubsub.publish("onesiteSettings.newRole");
            }
        };
    ...
    angular
        .module("settings")
        .service("onesiteNewRoleTabsManager", [
            "$rootScope",
            "timeout",
            "pubsub",
            "OnesiteNewRoleTabsData",
            "onesiteNewRoleTabsMenu",
            "onesiteNewRoleAside",
            "onesiteNewRoleModel",
