...
(function(angular, undefined) {
    ...
    function SpndMgmtAssignRoleTabsManager($rootScope, timeout, pubsub, tabsData, tabsMenu, aside, user, assignRoleModel, AssignRoleSaveSvc, persona) {
        var svc = this,
            triggerID = "SpndMgmtAssignRoleTabsManager";
            ...
            if (svc.reqCount !== 0) {
                ...
                if (assignRoleModel.currentType === "edit") {
                    ...
                    //if (svc.checkSelected() === true) {
                        // svc.assignRole();
                        svc.updateAllTabs(assignRoleModel);
                    //}
                } else {
                    //if (svc.checkSelected() === true) {
                        svc.updateAllTabs(assignRoleModel);
        ...
        svc.onUpdateRoleSuccess = function(resp) {
            if (!angular.isUndefined(resp.errorReason) && resp.errorReason.trim().length === 0 && resp.isError === false) {
                ...
                pubsub.publish("settings.spAssignRoleError", resp);
            }
        };

        svc.onCreateRoleError = function(resp) {
            pubsub.publish("settings.spAssignRoleError", resp.data);
        };
        ...
        svc.onUpdateSuccess = function() {
            ...
            if (svc.reqCount === svc.reqSuccessCount) {
                aside.hide();
                svc.resetCounts();
                pubsub.publish("spndMgmtSettings.editRole");
            }
        };
        ...
        svc.updateAllTabs = function(assignRoleModel) {
            angular.forEach(svc.tabs, function(item, id) {

                if (item.ctrl.state.id === "01") {
                    svc.reqCount++;
                    item.ctrl.onUpdate(assignRoleModel)
    ...
    angular
        .module("settings")
        .service("spndMgmtAssignRoleTabsManager", [
            "$rootScope",
            "timeout",
            "pubsub",
