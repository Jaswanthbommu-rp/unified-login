...
(function(angular, undefined) {
    ...
    function UserMgmtAssignRoleTabsManager($rootScope, timeout, pubsub, tabsData, tabsMenu, aside, user, assignRoleModel, assignRoleSaveSvc, persona) {
        var svc = this,
            triggerID = "UserMgmtAssignRoleTabsManager";
        ...
        svc.onUpdateRoleSuccess = function(resp) {
            if (!angular.isUndefined(resp.errorReason) && resp.errorReason.trim().length === 0 && resp.isError === false) {
                ...
                svc.updateAllTabs(assign);
            } else {
                //pubsub.publish("settings.assignRoleError", resp);
            }
        };

        svc.onCreateRoleError = function(resp) {
            pubsub.publish("settings.assignRoleError", resp.data);
        };
        ...
        svc.onUpdateSuccess = function() {
            ...
            if (svc.reqCount === svc.reqSuccessCount) {
                aside.hide();
                svc.resetCounts();
                pubsub.publish("userMgmtSettings.editRole");
            }
        };
    ...
    angular
        .module("settings")
        .service("userMgmtAssignRoleTabsManager", [
            "$rootScope",
            "timeout",
            "pubsub",
