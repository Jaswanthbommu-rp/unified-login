...
(function(angular, undefined) {
    ...
    function SpndMgmtCloneRoleTabsManager($rootScope, timeout, pubsub, tabsData, tabsMenu, aside, cloneRoleModel, cloneRoleSvc, user, cloneTabsContext,persona) {
        var svc = this,
        ...
        svc.cloneRole = function() {
            var data = {
                ...
                "roleDesc": cloneRoleModel.data.cloneRoleDesc,
                "inheritRoleId": cloneTabsContext.get().data.id
            };
            svc.updateAllTabs(data);
        ...
        svc.onCloneRoleSuccess = function(resp) {
            if (!angular.isUndefined(resp.errorReason) && resp.errorReason.trim().length === 0 && resp.isError === false) {
                ...
                pubsub.publish("spSettings.cloneRoleError", resp);
            }
        };

        svc.onCloneRoleError = function(resp) {
            pubsub.publish("spSettings.cloneRoleError", resp.data);
        };
        ...
        svc.onUpdateSuccess = function() {
            ...
            if (svc.reqCount === svc.reqSuccessCount) {
                aside.hide();
                svc.resetCounts();
                pubsub.publish("spndMgmtSettings.cloneRole");
            }
        };
    ...
    angular
        .module("settings")
        .service("spndMgmtCloneRoleTabsManager", [
            "$rootScope",
            "timeout",
            "pubsub",
            "spndMgmtCloneTabsData",
            "spndmgmtCloneRoleTabsMenu",
            "spndmgmtCloneRoleAside",
            "spndmgmtCloneRoleModel",
