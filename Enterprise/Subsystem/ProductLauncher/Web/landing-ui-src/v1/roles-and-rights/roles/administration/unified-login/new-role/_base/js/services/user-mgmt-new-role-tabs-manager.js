...
(function(angular, undefined) {
    ...
    function UserMgmtNewRoleTabsManager($rootScope, timeout, pubsub,
        tabsData, tabsMenu, aside, newRoleModel, newRoleSvc, user, persona) {
        var svc = this,
            triggerID = "UserMgmtNewRoleTabsManager";
        ...
        svc.init = function() {
            ...
            svc.updateWatch = pubsub.subscribe("newRole.update", svc.onUpdate);
            svc.cancelWatch = pubsub.subscribe("newRole.cancel", svc.onCancel);
        ...
        svc.onCreateRoleSuccess = function(resp) {
            ...
            if (!angular.isUndefined(resp.errorReason) && resp.errorReason.trim().length === 0 && resp.isError === false) {
                ...
                pubsub.publish("umSettings.newRoleError", resp);
            }
        };
        ...
        svc.onUpdateSuccess = function() {
            ...
            if (svc.reqCount === svc.reqSuccessCount) {
                aside.hide();
                svc.resetCounts();
                pubsub.publish("userMgmtSettings.newRole");
            }
        };
    ...
    angular
        .module("settings")
        .service("userMgmtNewRoleTabsManager", [
            "$rootScope",
            "timeout",
            "pubsub",
