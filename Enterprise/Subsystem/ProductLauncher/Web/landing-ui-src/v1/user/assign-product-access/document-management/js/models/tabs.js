//  Document Management Tabs Model

(function (angular, undefined) {
    "use strict";

    function factory(tabsMenu, tabsData) {
        function DocumentManagementTabsModel() {
            var s = this;
            s.init();
        }

        var p = DocumentManagementTabsModel.prototype;
        ...
        p.setTabs = function (keys) {
            ...
            keys.forEach(function (key) {
                if (tabsData[key]) {
                    ...
                    logw("key " + key + " is not a valid key!");
                }
            });
        ...
        p.reset = function () {
            ...
            angular.forEach(tabsData, function (val, key) {
                val.isActive = key === "roles";
            });
        };

        return new DocumentManagementTabsModel();
    }

    angular
        .module("settings")
        .factory("DocumentManagementTabsModel", [
            "rpScrollingTabsMenuModel",
            "docManagementTabsData",
            factory
        ]);
})(angular);
