...
var buildIdentityProviderPage = function buildIdentityProviderPage() {
    ...
    var settingPageID = $.sessionStorage.get('settingsPages');
    setCompanySettingUrl = "/setting/companysettings?CompanyId=" + companyId + "&CompanyName=" + companyName;

    var obj = settingPageID.find(function (obj) {
    ...
    //select the type
    var selectedType = function selectedType(optionData) {
    ...
    $('#IdentityProviderType').on('change', function () {
        $('.ipType').empty();
        var selectedValue = $('#IdentityProviderType').find('.dropdown-title').text().trim();
        $('#IdentityProviderType').find('.dropdown-item').removeClass('checked');
    ...
    var buildForm = function buildForm(selectedValue, JsonData) {
        ...
        $.each(JsonData, function (index, value) {
            ...
            if (value.type.trim() === selectedValue) {
                ...
                $.each(value.controlsList, function (ind, inputcontrols) {
                    var formcontrol = inputcontrols.controlType.toLowerCase();
    ...
    return Promise.resolve(userAPIService('GET', '', 'api/Pages/' + obj.id + '/PartialPages?companyId=' + companyId + '', '')).then(function (response) {
        ...
        var IdentityProviderPageId = response.find(function (page) {
            //debugger;
...
$(document).on('click', '#identity-provider-save', function () {
    ...
    var savObj = responseData.find(function (obj) {
    ...
    for (var j = 0; j < savObj.controlsList.length; j++) {
        var formcontrol = savObj.controlsList[j].controlType.toLowerCase();
