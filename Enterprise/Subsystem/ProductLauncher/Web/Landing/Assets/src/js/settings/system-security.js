...
let buildLeftMenu = (leftNavigationLinks) => {
    let leftMenu = $('.systerm-security-tabs');
    let content = $('.system-security-content');

    leftNavigationLinks.sort((a, b) => a.sequence - b.sequence).forEach((link, i) => {
        let menuItem = $('.system-security-tab-template.hidden').clone();
        let menuItemLink = menuItem.find('a');
        let linkIdValue = link.linkText.split(' ').join('-').toLowerCase();
...
$(document).ready(function () {
        ...
        Promise.resolve(settingsPages).then((settingsPages) => {
            ...
            let { breadcrumb, name, leftNavigationLinks } = page;
    ...
    $('#unified-platform-setup-wrap').on('submit', function (event) {
        ...
        let validDaysField = $('#password-validate-field');
        let passMinCharField = $('#password-min-characters');
        let passExpireField = $('#password-expire-field');
        let userTimeoutField = $('#user-timeout');
        let uniformPlatformsetupUrl = "api/UnifiedPlatformSetup";
        let AccessToggleValue = () => $('#companyAccessToggle').is(':checked');
        let linkValidDays = validDaysField.val();

        let minCharacterForRealPagePwd = Math.round(parseFloat(passMinCharField.val()));
        let noOfDaysBeforeCurrentPwdExpire = Math.round(parseFloat(passExpireField.val()));
        let unifiedPlatformUserTimeOut = Math.round(parseFloat(userTimeoutField.val()));
        ...
        let upsReqDto = {
            ...
            companyId,
            minCharacterForRealPagePwd,
            noOfDaysBeforeCurrentPwdExpire,
            unifiedPlatformUserTimeOut,
        };
...
function unifiedPlatformSetup() {
    ...
    let uniformPlatformsetupUrl = "api/UnifiedPlatformSetup?companyId=" + companyId;
...
function populateUnifiedPlatformData(response) {
    let {  blockCompanyProductLearningPortal,
           tempPwdNewUserLinkValidDays,
           noOfDaysBeforeCurrentPwdExpire,
           minCharacterForRealPagePwd,
           unifiedPlatformUserTimeOut } = response[0];
...
var tempPwdDecimalValidate = function () {
