...
productRequirements["OneSite"] = {
    ...
    access : [
        { name:"properties", data:"properties", selection:"multi", additionalParameters:"", title:"Properties" },
        { name:"roles", data:"roles", selection:"multi", additionalParameters:"", title:"Roles", rightsUrl:"onesite/rights"}
    ],
    status : "active"
};
productRequirements["RealPage Accounting"] = {
    ...
    access : [
        { name:"properties", data: "properties", selection:"multi", additionalParameters:"", title: "Entities / Groups" },
        { name:"roles", data: "roles", selection:"multi", additionalParameters:"", title: "Roles", rightsUrl:""}
    ],
    status : "active"
};
productRequirements["Spend Management"] = {
    apiName : "ops",
    access : [
        { name:"propertyGroups", data: "assets", selection:"single", additionalParameters:"", title: "Property Groups"},
        { name:"roles", data: "roles", selection:"single", additionalParameters:"", title: "Roles", rightsUrl:""}
    ],
    status : "active"
};
productRequirements["Vendor Services"] = {
    ...
    access : [
        { name:"propertyGroups", data: "propertygroups", selection:"single", additionalParameters:"", title: "Property Groups"},
        { name:"properties", data: "properties", selection:"single", additionalParameters:"", title: "Properties"},
        { name:"roles", data: "roles", selection:"single", additionalParameters:"accessType=Property", title: "Roles", rightsUrl:""},
        { name:"notifications",
            data: "notifications",
            descriptions: [
                { name: "isInsuranceExpired", description:"Notify by email when any vendor's insurance is about to expire." },
                { name:"isVendorRecommendationChanges", description:"Notify by email when any vendor's recommendation changes." }
            ],
...
productRequirements["Resident Portal"] = {
    ...
    access : [
        { name:"properties", data: "properties", selection:"multi", additionalParameters:"", title: "Properties"},
        { name:"roles", data: "levels", selection:"single", additionalParameters:"", title: "Roles", rightsUrl:""},
        { name:"messagingGroups", data: "messagegroups", selection:"multi", additionalParameters:"", title: "Messaging Groups"},
        { name:"notifications",
            data: "notifications",
            descriptions: [
                { name:"managerFdiViaEmail", description:"Front desk instructions." },
                { name:"amenitiesViaEmail", description:"New amenity reservation." },
                { name:"managerMrViaEmail", description:"Service request submission & updates." }
            ],
            additionalParameters:"",
            title: "Notifications"
        }
...
productRequirements["Lead2Lease"] = {
    apiName : "lead2lease",
    access : [
        { name:"propertyGroups", data: "properties", selection:"multi", additionalParameters:"", title: "Properties"},
        { name:"roles", data: "roles", selection:"multi", additionalParameters:"", title: "Rights", rightsUrl:""}
    ],
    status : "active"
};
...
productRequirements["Unified Login"] = {
    ...
    access : [
        { name:"roles", data: "roles", selection:"single", additionalParameters:"partyId=userPersona.organizationPartyId", title: "Roles", rightsUrl:"unifiedlogin/role/rights"}
    ],
    status : "active"
};
productRequirements["Client Portal"] = {
    ...
    access : [
        { name:"properties", data: "properties", selection:"multi", additionalParameters:"", title: "Properties"},
        { name:"roles", data: "roles", selection:"single", additionalParameters:"", title: "Roles", rightsUrl:"clientportal/role/rights"}
    ],
    status : "active"
};
...
var getPropertyData = function(response){
...
var getRoleData = function(response){
    ...
    if(roleData !== null) {
        ...
        $.each(roleData, function (i, role) {
            ...
            datatable.fnAddData([
                ...
                "<div class=\"valign-middle\">\n" + role.name + "</div>",
                "<div class=\"valign-middle\">\n" + role.roletype + "</div>",
                "<div class=\"valign-middle text-right " + showInfo + "\"><span class=\"fa fa-info-circle text-primary\" onclick=\"getRoleRights('" + role.name + "'," + role.id + ")\"></span></div>"
            ]);
        });
    }
...
var getPropertyGroupData = function(response){
...
var getMessageGroupData = function(response){
...
var getNotificationsData = function(response){
...
var addUserAccount = function(response){
...
var rpCreateUser = function(response) {
...
var rpGetUserTypes = function(response) {
...
var selectedProduct = function(name){
...
var buildAccessUrl = function(api, data, parameters, persona, editedUserId){
...
var buildRightsUrl = function(api, parameters, persona){
    if(parameters.length > 0){
...
var getRoleRights = function(name, roleId){
...
var showRightsData = function(response){
...
var validatePassword = function() {
    ...
    else if($("#user-details-password").val().length < 8 ||
        $("#user-details-password").val().length > 20 ||
        !uppercaseChars.test($("#user-details-password").val()) ||
        !lowercaseChars.test($("#user-details-password").val()) ||
        !numChars.test($("#user-details-password").val()) ||
        !specialChars.test($("#user-details-password").val())) {
        isValid = false;
        $("#user-details-password").addClass("create-user-input-error");
...
var validateCheckPassword = function () {
    var isValid = true;
...
$("#user-type").change(function () {
    if($("#user-type").val() == 401) { // Regular User
        checkShowTabs();
    ...
    else if($("#user-type").val() == 402) { // Realpage System Administrator
        checkShowTabs();
        $("#username").text('Email (Username)');
        $("#password-fields").hide();
        $("#user-details-password").attr('type','hidden');
    ...
    else if($("#user-type").val() == 404) { // Regular User (No-Email)
        checkShowTabs();
...
$(document).on('click', '#product-access-options .md-check input[type=checkbox]', function( e ) {
    ...
    var arrayIndex = selectedAccess[productName][accessType]['data'].findIndex((option => option.id == optionId));
