...
namespace RP.Enterprise.Subsystem.ProductLauncher.Component.Landing.Logic.Helper
{
    public static class BatchHelper
    {
        ...
        public static IList<ProductBatch> CreateAoBatchRecords(DefaultUserClaim userClaim, long editorPersonaId, long newUserPersonaId, bool externalUser = false, bool usePrimaryProperties = false, ListResponse propertiesResponse = null)
        {
            ...
            foreach (var aoUserCompanyPropertyRoleDetail in aoUserCompanyPropertyRoleDetails)
            {
                ...
                if (propertiesResponse != null)
                {
                    ListResponse userAssignedProperties = null;
                    if (propertiesResponse.Records?.Count > 0) userAssignedProperties = GetUserAssignedPropertiesData(propertiesResponse);
                    if (userAssignedProperties == null || userAssignedProperties.Records == null || (userAssignedProperties != null && userAssignedProperties.Records?.Count == 0))
                    {
