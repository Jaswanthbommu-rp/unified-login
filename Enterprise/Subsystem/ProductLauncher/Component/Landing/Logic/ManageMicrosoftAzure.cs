    ...
    public class ManageMicrosoftAzure : IManageMicrosoftAzure
    {
        ...
        public AzureUser GetADUserInfo(string userName)
        {
            try
            {
                ...
                logger.Write(Serilog.Events.LogEventLevel.Debug, "{ActionName} - {state}", propertyValues: new object[] { "GetADUserInfo", $" url: /v1.0/users?$filter = userPrincipalName eq '{userName}' &$select = userPrincipalName,onPremisesSamAccountName,displayName,mail,userPrincipalName,id" });
            ...
            catch (Exception ex)
            {
                var logger = Log.Logger;
                logger = logger.ForContext("ProductModule", this.GetType());
                logger.Write(Serilog.Events.LogEventLevel.Error, ex, "{ActionName} - {state}", propertyValues: new object[] { "GetADUserInfo", "Error when attempting to get Azure user info" });
            }
