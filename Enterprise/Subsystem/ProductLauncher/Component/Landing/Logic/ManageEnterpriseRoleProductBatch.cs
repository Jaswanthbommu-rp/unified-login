...
namespace RP.Enterprise.Subsystem.ProductLauncher.Component.Landing.Logic
{
    ...
    public class ManageEnterpriseRoleProductBatch
    {
        ...
        public string GenerateEnterpriseRoleUserProductBatch(EnterpriseRoleBatch batch)
        {
            ...
            var batchGroup = enterpriseRoleProductRepository.CreateBatchProcessGroup();
            ...
            foreach (var product in roleTemplateNewProducts)
            {
                ...
                if (productEnabledForPrimaryProperty)
                {
                    ...
                    if (ProductEnumHelper.GetAoProductList().Contains((ProductEnum)product))
                    {
                        ...
                        aOproductListToCreate.Add(aoProductsBatch);
                        string inputAOJson=  BatchHelper.BundleAoProducts(batchRecords, batchGroup.BatchProcessorGroupId);
                        enterpriseRoleProductRepository.SaveAOProductBatch(batch, aOproductListToCreate, editorPersona, enterpriseRoleProductRepository, impersonatorUserLoginOnly, batchGroup.BatchProcessorGroupId, inputAOJson);
                    }
                    else
                    {
            ...
            foreach (var product in roleTemplateUpdatedProducts)
            {
                try
                {
                    ...
                    {
                        var batchRecords = BatchHelper.CreateAoBatchRecords(_userClaim, batch.EditorUserPersonaId, batch.SubjectUserPersonaId, isExternalUser, usePrimaryProperties, propertiesResponse);
                        ProductBatch aoProductsBatch = new ProductBatch
                        {
                            ProductId = (int)ProductEnum.AssetOptimizer,
                            StatusTypeId = 5,
