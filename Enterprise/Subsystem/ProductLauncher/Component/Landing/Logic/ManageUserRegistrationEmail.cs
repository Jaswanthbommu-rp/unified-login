        ...
        public bool SendNewUserRegistrationEmail(UserLoginOnly userLoginOnly, string companyName, int userTypeId, long organizationPartyId)
        {
                ...
                || userTypeId == UserTypeConstants.ExternalUser))
            {
                try
                {
                    ...
                    logger.Write(LogEventLevel.Information, "{methodName} - {state}", propertyValues: new object[] { "SendNewUserRegistrationEmail", $"email template generated - {userLoginOnly.RealPageId}" });
                    ...
                    if (cesEmail.EmailBody != null)
                    {
                        ...
                        logger = logger.ForContext("CorrelationId", correlationId);
                        logger.Write(LogEventLevel.Information, "{methodName} - {state}", propertyValues: new object[] { "SendNewUserRegistrationEmail", $"email body generated - {userLoginOnly.RealPageId}" });
                    }
                    ...
                    logger = logger.ForContext("CorrelationId", correlationId);
                    logger.Write(LogEventLevel.Information, "{methodName} - {state}", propertyValues: new object[] { "SendNewUserRegistrationEmail", message });
                ...
                catch (Exception ex)
                {
                    var logger = Log.Logger;
                    logger = logger.ForContext("CorrelationId", correlationId);
                    logger.Write(LogEventLevel.Error, ex, "{methodName} - {state}", propertyValues: new object[] { "SendNewUserRegistrationEmail", $"email generation failed - {userLoginOnly.RealPageId}" });
        ...
        public bool SendPasswordResetEmail(ProfileDetail profileDetail)
        {
            ...
            )
            {
                try
                {
                    ...
                    logger.Write(LogEventLevel.Information, "{methodName} - {state}", propertyValues: new object[] { "SendPasswordResetEmail", $"email template generated - {userLoginOnly.RealPageId}" });
                    ...
                    if (cesEmail.EmailBody != null)
                    {
                        ...
                        logger = logger.ForContext("ProductModule", this.GetType());
                        logger = logger.ForContext("CorrelationId", _userClaim.CorrelationId);
                        logger.Write(LogEventLevel.Information, "{methodName} - {state}", propertyValues: new object[] { "SendPasswordResetEmail", $"email body generated - {userLoginOnly.RealPageId}" });
                    }
                    ...
                    logger = logger.ForContext("CorrelationId", _userClaim.CorrelationId);
                    logger.Write(LogEventLevel.Information, "{methodName} - {state}", propertyValues: new object[] { "SendPasswordResetEmail", message });
                ...
                catch (Exception ex)
                {
                    var logger = Log.Logger;
                    logger = logger.ForContext("CorrelationId", _userClaim.CorrelationId);
                    logger.Write(LogEventLevel.Error, ex, "{methodName} - {state}", propertyValues: new object[] { "SendPasswordResetEmail", $"email generation failed - {userLoginOnly.RealPageId} error: {ex.Message}" });
