...
namespace RP.Enterprise.Subsystem.ProductLauncher.Component.Landing.Logic.ProductIntegration.ProductImplementation
{
	public sealed class ClickPayManagement : StandardV1ProductIntegration, IManageProductIntegration
	{
		...
		public override ListResponse GetProductRoles(RequestParameter dataFilter, string baseUrlAndQuery = null)
		{
			try
			{
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductRoles", $"EditorPersona id - {EditorUserDetails.PersonaId}. At beginning of the method." });

				baseUrlAndQuery = string.Format(GetOperationEndPoint(ProductEntityEndpointKeyEnum.GetRoleEndpoint), CompanyInstanceSourceId);

				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductRoles", $"EditorPersona id - {EditorUserDetails.PersonaId}. At API calling - {baseUrlAndQuery}" });
                ...
                WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductRoles", $"EditorPersona id - {EditorUserDetails.PersonaId}. Resetting OrgsAssignedCount to 0 " });
                ...
                {
                    WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductRoles", $"EditorPersona id - {EditorUserDetails.PersonaId}. Calling GetProductUser for subject persona Id -{SubjectUserDetails.PersonaId}" });
                    ...
                    if (user != null)
                    {
                        WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductRoles", $"EditorPersona id - {EditorUserDetails.PersonaId}. Calling Merge OrgsAssignedCount for subject persona Id -{SubjectUserDetails.PersonaId}" });
			...
			{
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetProductRoles", $"EditorPersona id - {EditorUserDetails.PersonaId}. Error - {ex.Message}" }, exception: ex);
				return new ListResponse()
				{
		...
		public override ListResponse GetProductOrganizations(string organizationRoleId, string organizationType, string baseUrlAndQuery = null)
		{
			try
			{
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductOrganizations", $"EditorPersona id - {EditorUserDetails.PersonaId}. At beginning of the method." });
				...
				if (string.Equals(organizationType, "company", StringComparison.CurrentCultureIgnoreCase))
				{
                    ...
                    WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductOrganizations", $"EditorPersona id - {EditorUserDetails.PersonaId}. At API calling - {baseUrlAndQuery}" });
                    ...
                    WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductOrganizations", $"EditorPersona id - {EditorUserDetails.PersonaId}. Received roleList with count = {returnOrgList.Count}" });
                }
				else
				{
                    baseUrlAndQuery = string.Format(GetOperationEndPoint(ProductEntityEndpointKeyEnum.GetParentCompanyEndpoint), CompanyInstanceSourceId);
				...
				{
					WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductOrganizations", $"EditorPersona id - {EditorUserDetails.PersonaId}. Calling GetUser for subject persona Id -{SubjectUserDetails.PersonaId}" });
					...
					if (user != null)
					{
						WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductOrganizations", $"EditorPersona id - {EditorUserDetails.PersonaId}. Calling Merge for subject persona Id -{SubjectUserDetails.PersonaId}" });
			...
			{
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetProductOrganizations", $"EditorPersona id - {EditorUserDetails.PersonaId}. Error - {ex.Message}" }, exception: ex);
				return new ListResponse()
				{
					ErrorReason = ex.Message,
					IsError = true
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductUser", $"EditorPersona id - {EditorUserDetails.PersonaId}. At beginning of the method." });
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductUser", $"EditorPersona id - {EditorUserDetails.PersonaId}. Calling API - {baseUrlAndQuery}." });
		...
		public override ListResponse GetMigrationUsers(RequestParameter datafilter)
		{
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetMigrationUsers", "Received users from product." });
		...
		public override string UnassignUser()
		{
			...
			WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "UnassignUser", $"EditorPersona id - {EditorUserDetails.PersonaId}. DeleteUser returns fail" }, logData: logData);
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "ExternalProductUserProfileChange", $"EditorPersona id - {EditorUserDetails.PersonaId}, productUserProfile.UserId - {productUserProfile.UserId}. At beginning of the method." });
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "ExternalProductUserProfileChange", $"EditorPersona id - {EditorUserDetails.PersonaId}  productUserProfile.UserId - {productUserProfile.UserId}. Calling API - {baseUrlAndQuery}." });
			...
			WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "ExternalProductUserProfileChange", $"EditorPersona id - {EditorUserDetails.PersonaId} productUserProfile.UserId - {productUserProfile.UserId}. Result received - {result}." });
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "ProductUserProfileChange", $"EditorPersona id - {EditorUserDetails.PersonaId}, productUserProfile.UserId - {productUserProfile.UserId}. At beginning of the method." });
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "ProductUserProfileChange", $"EditorPersona id - {EditorUserDetails.PersonaId}  productUserProfile.UserId - {productUserProfile.UserId}. Calling API - {baseUrlAndQuery}." });
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "DeleteUser", $"EditorPersona id - {EditorUserDetails.PersonaId}. At beginning of the method." });

			// patch to se isActive flag to false
			var baseUrlAndQuery = GetOperationEndPoint(ProductEntityEndpointKeyEnum.PutUserEndpoint);

			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "DeleteUser", $"EditorPersona id - {EditorUserDetails.PersonaId}. Calling API - {baseUrlAndQuery}." });
