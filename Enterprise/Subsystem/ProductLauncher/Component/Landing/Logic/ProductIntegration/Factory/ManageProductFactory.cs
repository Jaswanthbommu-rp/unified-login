    ...
    public static class ManageProductFactory
    {
        ...
        public static IManageProductIntegration GetProductLogic(ProductEnum productType, long editorPersonaId, long subjectPersonaId, DefaultUserClaim userClaims)
        {
            ...
            catch (Exception ex)
            {
                try
                {
                    Log.Write(LogEventLevel.Error, ex, "{methodName} - {state}", propertyValues: new object[] { "GetProductLogic", $"Exception in Factory, mostly causes if exception in constructor. Message={ex.Message}" });
                }
                catch (Exception)
                { }
