...
namespace RP.Enterprise.Subsystem.ProductLauncher.Component.Landing.Logic.Product
{
    ...
    public class ManageProductAssetOptimization : ManageProductBase, IManageProductAssetOptimization
	{
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetCompanies", $"Beginning of method for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId} for product {productName}" });
			...
			try
			{
				...
				if (result.IsError)
				{
					WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetCompanies", $"GetCompanyEditorAndUserDetails error for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId} for product {productName} - {result.ErrorReason}" });
					return result;
				}
				...
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetCompanies", $"Exiting method with total rows - {response.TotalRows} for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId}." });
			}
			catch (Exception ex)
			{
				response.IsError = true;
				response.ErrorReason = "There was a problem getting the Companies.";
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetCompanies",$"Error for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId} " +
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetCompaniesWithRoles", $"Beginning of method for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId}" });
			...
			try
			{
				...
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetCompaniesWithRoles", $"Exiting method with total rows - {response.TotalRows} for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId} for product {productName}." });
			}
			catch (Exception ex)
			{
				response.IsError = true;
				response.ErrorReason = "There was a problem getting the GetCompaniesWithRoles.";
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetCompaniesWithRoles", $"Error for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId} for product {productName}" }, exception: ex);
			}
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductRoles", $"Beginning of method for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId}" });
			...
			try
			{
				...
				if (result.IsError)
				{
					WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetProductRoles", $"GetCompanyEditorAndUserDetails error for user with editorPersona id - {editorPersonaId} - {result.ErrorReason}" });
					return result;
				}
				...
				if (string.IsNullOrEmpty(aoCompanyId))
				{
					...
					WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductRoles", "Error looking for company id in bluebook." });
					return result;
				}
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductRoles", $"Found blue book company source id {aoCompanyId}" });
				...
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductRoles", $"Exiting method with total rows - {response.TotalRows} for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId} for product {productName}." });
			}
			catch (Exception ex)
			{
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetProductRoles", $"Error for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId} for product {productName}" }, exception: ex);
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetCompaniesWithProperties", $"Beginning of method for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId} for product {productName}." });
			...
			try
			{
				...
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetCompaniesWithProperties", $"Exiting method with total rows - {response.TotalRows} for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId} for product {productName}." });
			}
			catch (Exception ex)
			{
				response.IsError = true;
				response.ErrorReason = "There was a problem getting the GetCompaniesWithProperties.";
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetCompaniesWithProperties", $"Error for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId} for product {productName}." }, exception: ex);
			}
        ...
        {
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetOperators", $"Beginning of method for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId}." });
            ...
            try
            {
                ...
                if (result.IsError)
                {
                    WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetOperators", $"GetCompanyEditorAndUserDetails error for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId} - {result.ErrorReason}" });
                    return result;
                }
                CustomerCompanyMap company = GetProductCompanyInstanceId(_udmSourceCode);
                string aoCompanyId = company.CompanyInstanceSourceId;
                ...
                WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetOperators", $"Exiting method with total rows - {response.TotalRows} for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId}." });
            }
            catch (Exception ex)
            {
                response.IsError = true;
                response.ErrorReason = "There was a problem getting the GetOperators.";
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetOperators", $"Error for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId}." }, exception: ex);
            }
        ...
        {
            WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetPropertiesWithOperators", $"Beginning of method for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId}." });
            Dictionary<string, bool> allProperties = new Dictionary<string, bool>();
            var response = new ListResponse();
            try
            {
                ...
                if (result.IsError)
                {
                    WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetPropertiesWithOperators", $"GetCompanyEditorAndUserDetails error for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId} - {result.ErrorReason}" });
                    return result;
                }
                CustomerCompanyMap company = GetProductCompanyInstanceId(_udmSourceCode);
                string aoCompanyId = company.CompanyInstanceSourceId;
                ...
                WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetPropertiesWithOperators", $"Exiting method with total rows - {response.TotalRows} for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId}." });
            }
            catch (Exception ex)
            {
                response.IsError = true;
                response.ErrorReason = "There was a problem getting the GetPropertiesWithOperators.";
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetPropertiesWithOperators", $"Error for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId}." }, exception: ex);
            }
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductProperties", $"Beginning of method for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId} for product {productName}." });
			...
			try
			{
				...
				if (result.IsError)
				{
					WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetProductProperties", $"GetCompanyEditorAndUserDetails error for user with editorPersona id - {editorPersonaId} - {result.ErrorReason}" });
					return result;
				}
				...
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductProperties", $"Found blue book company source id {aoCompanyId}" });
				...
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductProperties", $"Exiting method with total rows - {response.TotalRows} for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId} for product {productName}." });
			}
			catch (Exception ex)
			{
				response = new ListResponse
				...
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetProductProperties", $"Error for user with editorPersona id - {editorPersonaId} and userPersonaId {userPersonaId} for product {productName}." }, exception: ex);
			}
		...
		public MigrateResponse UpdateUsersMigrationStatus(long editorPersonaId, IList<MigrateUser> migrateUsers)
		{
			...
			if (!migrateUsers.Any())
			{
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "UpdateUsersMigrationStatus", "Not updating status for AO External user." });
			...
			if (!string.IsNullOrWhiteSpace(response))
			{
				try
				{
					...
					WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "UpdateUsersMigrationStatus", "PutAsJsonAsync" }, logData: logData);
			...
			{
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "UpdateUsersMigrationStatus", $"PostAsJsonAsync" }, logData: logData);
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "ManageAssetOptimizationUser", $"Begin create/update user for user with editorPersona id - {editorPersonaId} and userPersonaId {productUserPersonaId}." });
			...
			try
			{
				...
				{
					WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "ManageAssetOptimizationUser", $"Error for user with editorPersona id - {editorPersonaId} and userPersonaId {productUserPersonaId}. Error - {listResponse.ErrorReason}" });
				...
				{
					WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "ManageAssetOptimizationUser", $"User Login Name not exist in greenbook for editorPersonaId - {editorPersonaId}." });
				...
				{
					WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "ManageAssetOptimizationUser", $"Error - Get CompanyMap - greenBookCares not enabled {blueAOCompanyInfo}" });
				...
				{
					WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "ManageAssetOptimizationUser", $"Error - No Valid Email Address Found - For User {productUserGbLogin.LoginName}" });
				...
				{
					WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "ManageAssetOptimizationUser", $"Begining realpage access user creation process with editorPersona id - {editorPersonaId} and userPersonaId {productUserPersonaId}." });
					...
					WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "ManageAssetOptimizationUser", $"Completed realpage access user creation process with editorPersona id - {editorPersonaId} and userPersonaId {productUserPersonaId}." });
				...
				else
				{
					...
					if (IsSuperUser(productUserPersonaId))
					{
						WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "ManageAssetOptimizationUser", $"User is super user with editorPersona id - {editorPersonaId} and userPersonaId {productUserPersonaId}." });
					...
					{					
						if (userAOProducts?.Count == 0)
						{
							...
							WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "ManageAssetOptimizationUser", $"Completed user creation process with editorPersona id - {editorPersonaId} and userPersonaId {productUserPersonaId}." });
						...
						else
						{
							...
							{
								WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "ManageAssetOptimizationUser", $"ERROR for user {productUserGbLogin.LoginName.ToLower()} while parsing AO PUT API response to check condition if all products removed. Result from API {_apiEndPoint}user/profile/{_editorProductUserId.ToLower()} is {returnResult}" });
								return returnResult;
							}
						}
					}				
			...
			{
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "ManageAssetOptimizationUser", $"Exception during user creation process with editorPersona id - {editorPersonaId} and userPersonaId {productUserPersonaId}." }, exception: ex);
				return ex.Message;
			}
		}
											   ...
											   UserLoginOnly productUserGbLogin)
		{
			...
			if (string.IsNullOrEmpty(biProductUserName))
			{
				...
				if (string.IsNullOrEmpty(createBIResult))
				{
					...
					WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "CreateUpdateAOBIProduct", $"Completed BI user creation process with editorPersona id - {editorPersonaId} and userPersonaId {productUserPersonaId}." });
					return createBIResult;
				}
				var jsObj = JsonConvert.DeserializeObject<dynamic>(createBIResult);
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "CreateUpdateAOBIProduct", $"Exception during BI user creation process with editorPersona id - {editorPersonaId} and userPersonaId {productUserPersonaId}." }, logData: jsObj, exception: null);
			}
			else
			{
				//	// Update User logic
				...
				if (string.IsNullOrEmpty(updateResult))
				{
					...
					WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "CreateUpdateAOBIProduct", $"Completed BI user update process with editorPersona id - {editorPersonaId} and userPersonaId {productUserPersonaId}." });
					return updateResult;
				}
				var jsObj = JsonConvert.DeserializeObject<dynamic>(updateResult);
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "CreateUpdateAOBIProduct", $"Exception during BI user update process with editorPersona id - {editorPersonaId} and userPersonaId {productUserPersonaId}." }, logData: jsObj, exception: null);
			}
			return result;
		}
		/// <summary>
		...
		public string UpdateUserProfile(long editorPersonaId, long userPersonaId)
		{
			...
			try
			{
				...
				if (string.IsNullOrEmpty(userEmailAddress))
				{
					WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "UpdateUserProfile", "Error.No Valid Notification Email Provided." });
					// write an error
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "CopyRegularUser", $"Begin - editorPersona id - {editorUserPersonaId}. sourceUserPersonaId {subjectUserPersonaId}" });
			...
			{
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "CopyRegularUser", $"Error -unable to find product User name with editorUserPersonaId   - {editorUserPersonaId}, subjectUserPersonaId {subjectUserPersonaId}." });
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "CopyRegularUser", $"End - editorPersona id - {editorUserPersonaId}. sourceUserPersonaId {subjectUserPersonaId}" });
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetPropertiesInGroup", $"Begin with editorPersona id - {editorPersonaId}." });
			...
			try
			{
				...
				if (listResponse.IsError)
				{
					WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetPropertiesInGroup", $"Error for user with editorPersona id - {editorPersonaId}. Error - {listResponse.ErrorReason}" });
					return listResponse;
				}
			...
			catch (Exception ex)
			{
				...
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetPropertiesInGroup", $"Error for user with editor AO user Id - {_editorProductUserId} " }, exception: ex);
			}
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetGroupProperties", $"Begin with editorPersona id - {editorPersonaId}." });
			...
			try
			{
				...
				if (listResponse.IsError)
				{
					WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetGroupProperties", $"Error for user with editorPersona id - {editorPersonaId}. Error - {listResponse.ErrorReason}" });
					return listResponse;
				}
			...
			catch (Exception ex)
			{
				response.IsError = true;
				response.ErrorReason = "There was a problem getting the AO products.";
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetGroupProperties", $"Error for user with editor AO user Id - {_editorProductUserId}" }, exception: ex);
			}
			...
			try
			{
				...
				if (selectedCompanies == null || selectedCompanies.Count == 0)
				{
					if (productName == "MA" || productName == "AX")
					{
						...
						WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetPropertyGroups", $"Received {groups.Count} groups for existing user." });
					}
				}
				...
				{
					WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetPropertyGroups", $"No groups received from product for user with editorPersona id - {editorPersonaId}." });
				...
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetPropertyGroups", $"Received {propertyGroups.Count} groups for existing user with editorPersona id - {editorPersonaId} & userPersonaId {userPersonaId}" });
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductPropertyGroups", $"Beginning of method for user with editorPersona id - {editorPersonaId}" });
			...
			try
			{
				...
				if (result.IsError)
				{
					WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetProductPropertyGroups", $"GetCompanyEditorAndUserDetails error for user with editorPersona id - {editorPersonaId} - {result.ErrorReason}" });
					return result;
				}
				...
				{
					result = new ListResponse { IsError = true, ErrorReason = "Company Setup Error: Please Contact Support." };
					WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductPropertyGroups", "Error looking for company id in bluebook." });
					return result;
				}
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductPropertyGroups", $"Found blue book company source id {aoCompanyId}" });
				...
				if (productName == "MA" || productName == "AX")
				{
					...
					WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductPropertyGroups", $"Received {groups.Count} groups for existing user." });
				}
				...
				if (propertyGroups == null || propertyGroups.Count == 0)
				{
					WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetProductPropertyGroups", $"No groups received from product for user with editorPersona id - {editorPersonaId}." });
				...
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductPropertyGroups", $"Received {propertyGroups.Count} groups for existing user with editorPersona id - {editorPersonaId} & userPersonaId {userPersonaId}" });
				...
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProductPropertyGroups", $"Exiting method with total rows - {response.TotalRows} for user with editorPersona id - {editorPersonaId}." });
			}
			catch (Exception ex)
			{
				response = new ListResponse
				...
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetProductPropertyGroups", $"Error for user with editorPersona id - {editorPersonaId}" }, exception: ex);
			}
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetGbSupportedAoEditorUserProductsToAssign", $"ManageProductAssetOptimization.GetGbSupportedAoUserProductsToAssign - Begin with editorPersona id - {userPersonaId}." });
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetGbSupportedAoEditorUserProductsToAssign", $"End of method for user with editorPersona id - {userPersonaId} samlProductUserName - {samlProductUserName}. productUserProfileApiUrl {productUserProfileApiUrl}, product count {products.Count}" });
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetGbSupportedAoProductsWithUserAdminRole", $"Begin with editorPersona id - {userPersonaId}." });
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetGbSupportedAoProductsWithUserAdminRole", $"End of method for user with editorPersona id - {userPersonaId} samlProductUserName - {samlProductUserName}. productUserProfileApiUrl {productUserProfileApiUrl}, product count {products.Count}" });
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetAOProductsForNewMultiCompanyUser", $"Begin with name - {loginName}." });
			...
			try
			{
				...
				if (objProductData != null)
				{
					...
					WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetAOProductsForNewMultiCompanyUser", $"End of method for user with ProductUserName - {loginName}. productUserProfileApiUrl {productUserProductApiUrl}, products {products.ToString()}" });
				}				
			}
			catch (Exception ex)
			{
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetAOProductsForNewMultiCompanyUser", $"Error for user {loginName} while getting AO Data from API {productUserProductApiUrl}" }, exception: ex);
			}
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "IsAOBIProductExistsInOtherOrganization", $"Begin with name - {loginName}." });
			...
			try
			{
				...
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "IsAOBIProductExistsInOtherOrganization", $"End of method for user with ProductUserName - {loginName}. productUserProfileApiUrl {productUserProductApiUrl}, products {aoUserProducts.ToString()}" });
			...
			{
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "IsAOBIProductExistsInOtherOrganization", $"Error for user {loginName} while getting AO Data from API {productUserProductApiUrl}" }, exception: ex);
			}
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetEditorUserAssignedCompaniesForProduct", $"Beginning of method for user with editorPersona id - {personaId}" });

			var samlProductUserName = GetSamlProductUserName(personaId).ToLower();

			if (string.IsNullOrEmpty(samlProductUserName))
			{
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetEditorUserAssignedCompaniesForProduct", $"Error -unable to find product User name with persona id - {personaId}." });
				return null;
			}
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetEditorUserAssignedCompaniesForProduct", $"End of method for user with editorPersona id - {personaId} editorSamlProductUserName - {samlProductUserName} productName {productName}" });
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetEditorUserAssignedPropertyGroups", $"Beginning of method for user with editorPersona id - {editorPersonaId}" });
			...
			{
				WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetEditorUserAssignedPropertyGroups", $"Error -unable to find product User name with persona id - {editorPersonaId}." });
				return null;
			}
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetSubjectUserAssignedPropertyGroups", $"Beginning of method for user with editorProductUserId - {editorProductUserId} productUserId {productUserId}" });
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetPropertiesInGroups", "Beginning of method." });

			var response = new List<AoProperty>();

			AoVisiblePropertyGroups visiblePropertyGroups = GetAllPropertyGroups();

			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetPropertiesInGroups", $"Received {visiblePropertyGroups.Groups.Count} groups for existing user." });
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetGroupProperties", "Beginning of method." });

			var response = new List<ProductProperty>();

			AoVisiblePropertyGroups visiblePropertyGroups = GetAllPropertyGroups();

			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetGroupProperties", $"Received {visiblePropertyGroups.Groups.Count} groups for existing user." });
        ...
        {
            WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetPropertyByGroupId", "Beginning of method." });
            List<VisibleGroupProperty> propertyGroups = new List<VisibleGroupProperty>();
			var response = new List<ProductProperty>();
            WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetPropertyByGroupId", "ManageProductAssetOptimization.GetAllPropertyGroups- Null cache value. Getting new Groups." });
            ...
            WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetPropertyByGroupId", $"Received {propertyGroups.Count} groups for existing user." });
            return response;
        }

        private AoVisiblePropertyGroups GetAllPropertyGroups()
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetAllPropertyGroups", "Beginning of method." });
			...
			{
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetAllPropertyGroups", "Null cache value. Getting new Groups." });
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetAllPropertyGroups", $"Received {propertyGroups.Groups.Count} groups for existing user." });
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetAssignablePropertyGroups", "Beginning of method." });
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetAssignablePropertyGroups", $"Received {result.Groups.Count} groups for existing user." });
		...
		private T GetResultFromApi<T>(string baseUrlAndQuery) where T : class
		{
			...
			using (var client = new HttpClient())
			{
				...
				{
					WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetResultFromApi", $"Error - Response is not 200. baseUrlAndQuery {baseUrlAndQuery}, StatusCode - {response.StatusCode}" });
				}
			}
        ...
        private string PostApi(string baseUrlAndQuery, object inputObject)
		{
			...
			using (var client = new HttpClient())
			{
				...
				else
				{
					...
					WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetResultFromApi", $"Error - Response is not 200. PostApi, baseUrlAndQuery {baseUrlAndQuery}, StatusCode - {response.StatusCode}, jsonContent {jsonContent}, errorResult {result}" });
				}
			}
			...
			using (var client = new HttpClient())
			{
				...
				using (var response = client.PutAsJsonAsync(baseUrlAndQuery, inputObject).Result)
				{
					...
					else
					{
						...
						WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetResultFromApi", $"Error - Response is not 200. PutApi, baseUrlAndQuery {baseUrlAndQuery}, StatusCode - {response.StatusCode}, jsonContent {jsonContent}, result {result}" });
					}
				}
			}
		...
		{
			// Default AO record
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "CreateProductUserInGreenBook", $"Inserting in GB -productUsername -{productLoginName} for AO user" });
			...
			{
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "CreateProductUserInGreenBook", $"Inserting in GB -productUsername -{productLoginName}, product - {product}." });
				...
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "CreateProductUserInGreenBook", $"Create user Success. Set product status to Success. productUsername -{productLoginName}, product - {product}" });
			}
		}
			...
			bool loginNameChanged = false)
		{
			...
			else
			{
				if (productAssigned.Any())
				{
					...
					WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "UpdateProductUserInGreenBook", $"Checking AO record in GB -productUsername -{productLoginName} for AO user" });
					...
					{
						WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "UpdateProductUserInGreenBook", $"No AO record found in GB for AO user -{productLoginName}. Creating new one." });
					...
					{
                        WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "UpdateProductUserInGreenBook", $"Checking AO record in GB - productUsername -{productLoginName} for AO user and exist product assigned loginName changed" });
					...
					{
						WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "UpdateProductUserInGreenBook", $"Checking if product {product} exists in GB for AO user - {productLoginName}" });
						samlUserDetails = _samlRepository.GetProductSamlDetails(userPersonaId,
							(int)ProductEnumHelper.GetAoProductEnum(product));

						if (!samlUserDetails.Any())
						{
							WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "UpdateProductUserInGreenBook", $"No {product} record found in GB for AO user -{productLoginName}. Creating new one." });
						...
						{
                            WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "UpdateProductUserInGreenBook", $"Checking AO record in GB - productUsername -{productLoginName} for AO user and product assigned loginName changed" });
						...
						WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "UpdateProductUserInGreenBook", $"{product} record updated in GB for AO user -{productLoginName}. (UpdateProductSettingProductStatus)" });
					}
				}
				...
				if (productUnAssigned.Any())
				{
					...
					foreach (var product in productUnAssigned)
					{
						...
						if (samlUserDetails.Any())
						{
							WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "UpdateProductUserInGreenBook", $"{product} record found in GB for AO user -{productLoginName}. Removing." });
							...
							WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "UpdateProductUserInGreenBook", $"{product} record removed from GB for AO user -{productLoginName}." });
						}
					}
				}
			}
		...
		private void AssociateAoUserWithGb(long editorPersonaId, long productUserPersonaId, string loginName, AOUser aoUser, CustomerCompanyMap customerCompanyMap)
		{
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "AssociateAoUserWithGb", $"EditorPersona id - {editorPersonaId} productUserPersonaId {productUserPersonaId}, Products - {string.Join<string>(",", products)}" });
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetAoProductsForUserNew", $"LoginName {loginName}" });
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetAoProductsForUserNew", $"Received products {products.Count} for user with loginName {loginName} API-URL {productUserProfileApiUrl}" });
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetAoProductsForUser", $"LoginName {loginName}" });
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetAoProductsForUser", $"Received products {products.Count} for user with loginName {loginName} API-URL {productUserProfileApiUrl}" });
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetRoles", $"Beginning of method for user with _editorProductUserId{_editorProductUserId} _productUserId {_productUserId} companyId - {companyId} productName {productName}" });
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetRoles", $"Received {allRoles.Count} roles for existing user with _editorProductUserId{_editorProductUserId} _productUserId {_productUserId}  companyId - {companyId} productName {productName}" });
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProperties", $"Beginning of method for user with _editorProductUserId{_editorProductUserId} _productUserId {_productUserId}  companyId - {companyId} productName {productName}" });
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProperties", $"Received {objAoPropertyList.Properties.Count} properties for new user with _editorProductUserId{_editorProductUserId} _productUserId {_productUserId}  companyId - {companyId} productName {productName}" });
			...
			if (!string.IsNullOrEmpty(productUserId))
			{
				...
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetProperties", $"Received {objAoPropertyList.Properties.Count} properties for existing user _editorProductUserId{_editorProductUserId} _productUserId {productUserId}  companyId - {companyId} productName {productName}." });
			}
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetGbSupportedAoSubjectProductsAssigned", "Begin." });
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetGbSupportedAoSubjectProductsAssigned", $"End of method - product count {products.Count}" });
		...
		{
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "CopyEditorUserToCreateSuperUser", $"Begin - sourceUserPersonaId id - {sourceUserPersonaId}." });
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "CopyEditorUserToCreateSuperUser", $"End - sourceUserPersonaId id - {sourceUserPersonaId}." });
		...
		private string GetUserEmailAddress(Guid realPageId, string logInName, long productUserPersonaId)
		{
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetUserEmailAddress", "Begin get user email address" });
			...
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetUserEmailAddress", "Got list of electronic address" });
			if (_addresses != null)
			{
				if (_addresses.Any(a => a.AddressType.ToUpper() == "EMAIL"))
				{
					...
					WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetUserEmailAddress", $"ManageProductAssetOptimization - Found email address. {userEmailAddress}" });
				}
			}
			...
			if (IsRegularUserNoEmail(productUserPersonaId))
			{
				if (string.IsNullOrEmpty(userEmailAddress))
				{
					WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetUserEmailAddress", "Error.No Valid Notification Email Provided." });
					// write an error
					return "ManageProductAssetOptimization - Error.No Valid Notification Email Provided";
				}
			...
			else if (string.IsNullOrEmpty(userEmailAddress))
			{
				...
				WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetUserEmailAddress", "Using login name for email address." });
			}

			// verify email address looks valid, will fail if not
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetUserEmailAddress", $"Validating email address. Email: {userEmailAddress}" });
			userEmailAddress = ValidateAndReturnEmailAddress(userEmailAddress);
			WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetUserEmailAddress", $"Validated email address. Email: {userEmailAddress}" });
