...
namespace RP.Enterprise.Subsystem.ProductLauncher.Component.Landing.Logic.Product
{
	public class ManageProductProspectContact : ManageProductBase, IManageProductProspectContact
	{
		...
		public string ManageProductProspectContactUser(long editorPersonaId, long userPersonaId, ProspectContactPropertyRole userProspectContactPropertyRole, BatchProcessType batchProcessType = BatchProcessType.CreateUpdateProductUser)
		{
			...
			try
			{
				...
				{
					prospectContactCenterUser.User.ManagementCompanyID = "0";
					prospectContactCenterUser.User.PropertyID = userProspectContactPropertyRole.PropertyList[0];
				...
				if (string.IsNullOrEmpty(_productUsername)) // NEW USER
				{
					...
					string newProductUserId = InsertProspectContactCenterUser($"{_apiEndPoint}/User", userPersonaId, editorPersonaId, productLoginName, );
		...
		private ListResponse MergeProductPropertiesWithGreenbook(IList<ProductProperty> propertyList)
		{
			...
			else if (prospectContactCenterUser.PropertyID != "0")
			{
				// if a user record exists
				ProductProperty pp = (from a in propertyList
									  where a.ID == prospectContactCenterUser.PropertyID
			...
			ProspectContactCenterUser prospectContactCenterUser)
		{
			...
			if (currentProspectContactCenterUser.PropertyID != prospectContactCenterUser.User.PropertyID)
			{
				...
				var resultUpdateUserProperty = UpdateUserProperty(prospectContactCenterUser.User.PropertyID,
					prospectContactCenterUser.ModifyingUser);
		...
		private string UpdateUserProperty(string propertyId, string modifyingUserId)
		{
			...
			userPropObj.PropertyID = propertyId;
	...
	public class ProspectContactCenterUser
	{
		...
		public ProspectContactCenterUserProfile User { get; set; }
	}
