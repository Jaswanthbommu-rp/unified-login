...
namespace RP.Enterprise.Subsystem.ProductLauncher.Component.Landing.Logic.Product
{
	...
	public class ManageProductMarketingCenter : ManageProductBase, IManageProductMarketingCenter
    {
		...
		public ManageProductMarketingCenter(DefaultUserClaim userClaims) : base((int)ProductEnum.MarketingCenter, userClaims, null)
        {
            ...
            : base((int)ProductEnum.MarketingCenter, userClaims, productInternalSettingRepository)
        {
            ...
            _productUrl = _productInternalSettingList.First(a => a.Name.ToUpper() == "APIENDPOINT").Value;
			_marketingCenterApiSourceID = _productInternalSettingList.First(a => a.Name.ToUpper() == "MARKETINGCENTERAPISOURCEID").Value;
			_username = Encoding.UTF8.GetString(Convert.FromBase64String(_productInternalSettingList.First(a => a.Name.ToUpper() == "APIUSERNAME").Value));
            _password = Encoding.UTF8.GetString(Convert.FromBase64String(_productInternalSettingList.First(a => a.Name.ToUpper() == "APIPASSWORD").Value));
            // TODO REMOVE WHEN POSTING TO TRUSTED URL
            ServicePointManager.ServerCertificateValidationCallback += (sender, cert, chain, sslPolicyErrors) => true;
                    ...
                    if (userPersonaId != 0)
                    {
                        ...
                        if (list.Any(a => a.ID == mUser.ContactRoleId.ToString()))
                        {
                            ProductRole pr = (from a in list
                                              where a.ID == mUser.ContactRoleId.ToString()
		...
		public ListResponse GetProperties(long editorPersonaId, long userPersonaId, RequestParameter datafilter)
		{
			...
			try
			{
                ...
                //IList<CompanyMap> companyMap = _blueBook.GetCompanyMap(_editorPersona.Organization.BooksMasterId, BlueBookProductConstants.MarketingCenter);
                IList<CustomerCompanyMap> companyMap = _blueBook.GetCompanyMap(_editorPersona.Organization.BooksCustomerMasterId, BlueBookProductConstants.MarketingCenter);
                WriteToDiagnosticLog("GetMarketingCenterPMCIDFromPersona - Done getting info from BlueBook.GetCompanyMap");
				if (companyMap != null && companyMap.Count > 0 && companyMap.Any(a => a.Source.ToUpper() == BlueBookProductConstants.MarketingCenter))
				{
					WriteToDiagnosticLog("GetMarketingCenterPMCIDFromPersona - Getting PMC ID from BlueBook result");
					marketingCenterCompanyId = companyMap.First(a => a.Source.ToUpper() == BlueBookProductConstants.MarketingCenter).CompanyInstanceSourceId;
                ...
                if (companyMap == null)
                {
                    ...
                    WriteToErrorLog(
                        $"GetMarketingCenterPMCIDFromPersona.GetProperties - Unable to get companyInstanceSourceId info in bluebook for user with editorPersona id - {editorPersonaId}.");
                    return new ListResponse { IsError = true, ErrorReason = "Company Setup Error: Please Contact Support." };
		...
		public string UpdateUserProfile(long editorPersonaId, long userPersonaId)
		{
			...
			try
			{
                ...
                if (_addresses != null)
                {
                    ...
                    {
                        userEmailAddress = (from a in _addresses where a.AddressType.ToUpper() == "EMAIL" 
				...
				if (userPersona.UserTypeId == (int)UserTypeConstants.RegularUserNoEmail)
				{
					userEmailAddress = !new System.ComponentModel.DataAnnotations.EmailAddressAttribute().IsValid(userLogin.LoginName) ?
										string.Concat(userLogin.LoginName, "@NoReply.com") : userLogin.LoginName;
				}
				else
				{
				...
				else
				{
					...
					{/*Ignored*/ }
					WriteToErrorLog(
						$"ManageMarketingCenterUser.UpdateUserProfile Error for user with editorPersona id - {editorPersonaId}.", logData);
		...
		public string ManageMarketingCenterUser(long editorPersonaId, long userPersonaId, List<int> RoleList, List<string> PropertyList)
        {
            ...
            if (!isSuperUser && (RoleList.Count == 0 || PropertyList.Count == 0))
            {
				if (RoleList.Count == 0) 
				{
					WriteActivityLogWithMessage(editorPersonaId, userPersonaId, "An error occurred when {3} {4} attempted to provision {2} for {0} {1}.There is are no roles active in this company.  Please contact the implementation team for this product.");
					UpdateProductSettingProductStatus(userPersonaId, _productSettingType_ProductStatus, (int)ProductBatchStatusType.Stop);
				}

				if (PropertyList.Count == 0)
				{
					WriteActivityLogWithMessage(editorPersonaId, userPersonaId, "An error occurred when {3} {4} attempted to provision {2} for {0} {1}.There is are no properties active in this company.  Please contact the implementation team for this product.");
					UpdateProductSettingProductStatus(userPersonaId, _productSettingType_ProductStatus, (int)ProductBatchStatusType.Stop);
				}
            ...
            if (isSuperUser)
            {
                ...
                if (roleList.Any(a => a.Name.ToUpper() == "CORPORATE OPERATIONS"))
                {
                    ProductRole p = (from a in roleList
                                     where
                                   a.Name.ToUpper() == "CORPORATE OPERATIONS"
                                   select a).FirstOrDefault();
                    if (p != null)
                    {
            ...
            else
            {
				...
				if (PropertyList[0].ToString().ToUpper() != "ALL")
				{
			...
			if (PropertyList != null && PropertyList.Count > 0 && PropertyList[0].ToString().ToUpper() == "ALL" && !isSuperUser)
			{
		...
		public bool ChangeUserStatus(long editorPersonaId, string username, string productUserId, bool isActive = false)
		{
			...
			{
				WriteToErrorLog(
					$"ManageProductMarketingCente.ChangeUserStatus - Error looking for company id in bluebook for user with editorPersona id - {editorPersonaId}.");
				return false;
			}
			try
			{
                if (!SetMarketingCenterUserStatus(isActive, editorPersonaId, _productUserId)){
                    return false;
                }
            }
			catch (Exception ex)
		...
		private bool SetMarketingCenterUserStatus(bool isActive, long editorPersonaId, string mcUserId)
		{
			...
			{				
                ...
                if (string.IsNullOrEmpty(_editorProductUserId)){
                    throw new Exception("_editorProductUserId cannot be null or empty");
                }
			...
			foreach (var user in migrateUsers){
				if (string.IsNullOrEmpty(user.LeadEmailAddress)){
					...
					if (_addresses != null)
					{
						if (_addresses.Any(a => a.AddressType.ToUpper() == "EMAIL"))
						{
							user.LeadEmailAddress = (from a in _addresses
        ...
        /*
        /// <summary>
        /// Used to update product status for the user
        /// </summary>
        /// <param name="userPersonaId"></param>
        /// <param name="status"></param>
        private void UpdateProductSetting(long userPersonaId, ProductBatchStatusType status)
        {
            ...
            if (productSettingTypes.Any(a => a.Name.ToUpper() == "PRODUCTSTATUS"))
            {
                int productStatusTypeId = (from a in productSettingTypes where a.Name.ToUpper() == "PRODUCTSTATUS" select a.ProductSettingTypeId).FirstOrDefault();
                repositoryResponse = _productRepository.CreateProductSetting(userPersonaId, _productId, productStatusTypeId, status.ToString());
            }
        }
        */
        ...
        /* /// <summary>
           ...
           /// </summary>
           /// <returns></returns>
           private CompanyMap GetMarketingCenterCompanyInstanceId2()
           {
               IList<CompanyMap> companyProductList = _blueBook.GetCompanyMap(_editorPersona.Organization.BooksMasterId, BlueBookProductConstants.MarketingCenter);
               CompanyMap company = new CompanyMap();
               if (companyProductList.Any(a => a.Source.ToUpper() == BlueBookProductConstants.MarketingCenter))
               {
                   company = (from a in companyProductList where a.Source.ToUpper() == BlueBookProductConstants.MarketingCenter select a).FirstOrDefault();
               }
               return company;
           }*/
