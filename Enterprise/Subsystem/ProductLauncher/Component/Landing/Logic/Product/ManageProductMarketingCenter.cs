    ...
    public class ManageProductMarketingCenter : ManageProductBase, IManageProductMarketingCenter
	{
		...
		public ListResponse GetRoles(long editorPersonaId, long userPersonaId, RequestParameter datafilter)
		{
			...
			try
			{
				...
				if (response.IsSuccessStatusCode)
				{
                    ...
                    WriteToDiagnosticLog("{methodName} - {state}", logData, messageProperties: new object[] { "GetRoles", "Got response" });
					...
					if (userPersonaId != 0)
					{
						...
						if (mUser == null)
						{
							WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetRoles", $"Error looking for user. userPersonaId={userPersonaId}" });
                            return new ListResponse() { IsError = true, ErrorReason = "User not found" };
						}
						logData = new Dictionary<string, object>();
						logData.Add("mUser", mUser);
						WriteToDiagnosticLog("{methodName} - {state}", logData, messageProperties: new object[] { "GetRoles", "Looking for role for user" });
                        ...
                        if (list.Any(a => a.ID == mUser.ContactRoleId.ToString()))
						{
							...
							{
								pr.IsAssigned = true;
								WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetRoles", $"Found role for user. Role: {pr.Name}" });
                            }
						}
					}
					logData = new Dictionary<string, object>();
					logData.Add("list", list);
					WriteToDiagnosticLog("{methodName} - {state}", logData, messageProperties: new object[] { "GetRoles", "Returning role list" });
				...
				else
				{
					...
					WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetRoles", "Error " + response.Content.ReadAsStringAsync().Result });
                }
			}
			catch (Exception ex)
            {
                WriteToErrorLog("{methodName} - {state}", exception: ex, messageProperties: new object[] { "GetRoles", $"Error. {ex.Message}" });
                result = new ListResponse();
				result.IsError = true;
		...
		public ListResponse GetProperties(long editorPersonaId, long userPersonaId, RequestParameter datafilter)
		{
			...
			try
			{
				...
				WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "GetProperties", "Getting info from BlueBook.GetCompanyMap" });
				...
				WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "GetProperties", "Done getting info from BlueBook.GetCompanyMap" });
				...
				{
					WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "GetProperties", "Getting PMC ID from BlueBook result" });
					...
					WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "GetProperties", $"Found PMC ID from BlueBook result: {marketingCenterCompanyId}" });
				...
				logData = new Dictionary<string, object> { { "url", url } };
                WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "GetProperties", "Posting to url" });
                var response = _httpClient.GetAsync(url).Result;
				...
				if (response.IsSuccessStatusCode)
				{
					...
					if (userPersonaId != 0)
					{
						...
						if (mUser == null)
						{
							WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "GetProperties", $"Error looking for user. userPersonaId={userPersonaId}" });
                            return new ListResponse() { IsError = true, ErrorReason = "User not found" };
						}
						logData = new Dictionary<string, object>();
						logData.Add("mUser", mUser);
						WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "GetProperties", "Looking for properties for user" });
                        ...
                        if (mUser.AssignedProperties != null)
						{
							...
							WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "GetProperties", "Adding extra property" });
                        }
                    ...
                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "GetProperties", "Returning property list" });
				...
				else
				{
					...
					WriteToErrorLog("{methodName} - {status}", messageProperties: new object[] { "GetProperties", $"Error. {response.Content.ReadAsStringAsync().Result}" });
                }
			}
			catch (Exception ex)
			{
				...
				WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "GetProperties", $"Error. {ex.Message}" });
            }
			return result;
		}
		...
		public string UnassignUser(long editorPersonaId, long userPersonaId)
		{
			...
			if (!IsUserIdValid(Convert.ToInt64(_editorProductUserId)))
			{
                ...
                WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "UnassignUser", $"Invalid admin userId: {_editorProductUserId}" });
            }
			else
			{
				...
				if (status)
				{
                    WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "UnassignUser", $"userPersonaId: {userPersonaId}" });
		...
		public string UpdateUserProfile(long editorPersonaId, long userPersonaId)
		{
			try
			{
                ...
                WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateUserProfile", "Begin update user profile" });
                string productLoginName = "";

				Persona userPersona = _managePersona.GetPersona(userPersonaId);
                WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateUserProfile", "Got persona info" });
                Guid realPageId = userPersona.RealPageId;

				Person person = _managePerson.GetPerson(realPageId);
                WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateUserProfile", "Got person info" });
                ...
                WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateUserProfile", "Begin get user email address" });
                ...
                WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateUserProfile", "Got list of electronic address" });
                if (_addresses != null)
				{
					if (_addresses.Any(a => a.AddressType.Equals("EMAIL", StringComparison.OrdinalIgnoreCase)))
					{
                        ...
                        WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateUserProfile", $"Found email address. {userEmailAddress}" });
                    }
				}
				if (string.IsNullOrEmpty(userEmailAddress))
				{
					userEmailAddress = userLogin.LoginName;
                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateUserProfile", "Using login name for email address" });
                }
				...
				WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateUserProfile", $"User Type : {userPersona.UserTypeId}" });
                WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateUserProfile", $"Validating email address. Email: {userLogin.LoginName}" });
                ...
                WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateUserProfile", $"Validated email address. Email: {userEmailAddress}" });
                WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateUserProfile", $"Product User Name : {_productUsername}" });
				...
				if (mUser == null)
				{
                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateUserProfile", $"Error looking for user. userPersonaId={userPersonaId}" });
                ...
                WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateUserProfile", "Update user profile" });
                var response = _httpClient.PutAsJsonAsync(url, mcUser).Result;

				if (response.IsSuccessStatusCode)
				{
					WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "UpdateUserProfile", $"StartUpdate user SAMLAttribute User_email={productLoginName}" });
                    UpdateSamlUserAttribute(userPersonaId, _productId, SamlAttributeEnum.productUsername, productLoginName);
					WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "UpdateUserProfile", "Update user SAMLAttribute User_email success. Saved user id" });
                ...
                {/*Ignored*/ }
                WriteToErrorLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateUserProfile", $"Error for user with editorPersona id - {editorPersonaId}" });
                return $"There was a problem updating user profile for user with editorPersona id - {editorPersonaId} - Error-{errorContent}.";
            }
			catch (Exception ex)
			{
				WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "UpdateUserProfile", $"Error. {ex.Message}, for user with editorPersona id - {editorPersonaId}" });
        ...
        public string ManageMarketingCenterUser(long editorPersonaId, long userPersonaId, List<int> RoleList, List<string> PropertyList, bool IsAssignedNewPropertyByDefault)
		{
			...
			WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", "Begin create/update user" });
            string productLoginName = "";

			Persona userPersona = _managePersona.GetPersona(userPersonaId);
			WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", "Got persona info" });
            Guid realPageId = userPersona.RealPageId;

			IC.IPerson person = _managePerson.GetPerson(realPageId);
			WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", "Got person info" });
			...
			// get the email address
			WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", "Begin get user email address" });
            string userEmailAddress = "";
			string userLeadEmailAddress = "";
			ManageElectronicAddress _manageElectronicAddress = new ManageElectronicAddress();
			IList<IC.ElectronicAddress> _addresses = _manageElectronicAddress.ListElectronicAddressForPerson(userLogin.RealPageId, "");
			WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", "Got list of electronic address" });
            if (_addresses != null)
			{
				if (_addresses.Any(a => a.AddressType.Equals("EMAIL", StringComparison.OrdinalIgnoreCase)))
				{
					userEmailAddress = (from a in _addresses where a.AddressType.Equals("EMAIL", StringComparison.OrdinalIgnoreCase) select a.AddressString).FirstOrDefault();
					WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", $"Found email address. {userEmailAddress}" });
                }
			}
			if (IsRegularUserNoEmail(userPersonaId))
			{
				...
				{
					WriteToErrorLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", "No Valid Notification Email Provided" });
                    // write an error
			...
			else
			{
				...
				{
					userEmailAddress = userLogin.LoginName;
					WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", "Using login name for email address" });
                }
			}

			// verify email address looks valid, will fail if not
			WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", $"Validating email address. Email: {userEmailAddress}" });
            userEmailAddress = ValidateAndReturnEmailAddress(userEmailAddress);
			WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", $"Validated email address. Email: {userEmailAddress}" });
			...
			if (!isSuperUser && (RoleList.Count == 0 || PropertyList.Count == 0))
			{
				...
				WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", $"Create user error. PropertyList.Count={PropertyList.Count} , RoleList.Count={RoleList.Count}" });
                // stop batch process
			...
			if (isSuperUser)
			{
				...
				if (roleId == 0)
				{
                    ...
                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "ManageMarketingCenterUser", "Error getting role id for Corporate Operations for super user" });
			...
			else
			{
				...
				{
					logData = new Dictionary<string, object> { { "roleList", roleList } };
                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "ManageMarketingCenterUser", $"Error getting role id {RoleList[0]} from role list" });
                    // write an error
            ...
            WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", $"Using product login name. productUsername: {_productUsername}" });
            MC.MarketingCenterUser mcUser = new MC.MarketingCenterUser();
			CustomerCompanyMap company = GetProductCompanyInstanceId(_udmSourceCode);

			if (string.IsNullOrEmpty(company.CompanyInstanceSourceId))
			{
				WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", "Error looking for company id in bluebook" });
                // write an error
            ...
            WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "ManageMarketingCenterUser", "User details" });
            ...
            if (string.IsNullOrEmpty(_productUsername))
			{
				...
				try
				{
                    ...
                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "ManageMarketingCenterUser", "Create user" });
					...
					if (response.IsSuccessStatusCode)
					{
                        ...
                        WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "ManageMarketingCenterUser", "Create user. Got result from marketing center" });
						...
						WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", $"Create user. newid={newid}" });
                        _samlRepository.CreateSamlUserAttribute(userPersonaId, _productId, SamlAttributeEnum.UserId, newid.ToString());
						WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", "Create user. Saved user id" });
                        UpdateProductSettingProductStatus(userPersonaId, _productSettingType_ProductStatus, (int)ProductBatchStatusType.Success);
						WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", "Create user. Set product status to Success" });
				...
				{
					// write an error
					WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "ManageMarketingCenterUser", $"Create user errored. {ex.Message}" });
                    UpdateProductSettingProductStatus(userPersonaId, _productSettingType_ProductStatus, (int)ProductBatchStatusType.Error);
                    WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "ManageMarketingCenterUser", "Create user errored. Set product status to Error" });
			...
			else
			{
				...
				try
				{
                    ...
                    logData = new Dictionary<string, object> { { "url", url }, { "userJson", JsonConvert.SerializeObject(mcUser) } };
                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "ManageMarketingCenterUser", "Update user" });
					...
					if (response.IsSuccessStatusCode)
					{
						...
						var userResult = JsonConvert.DeserializeObject<dynamic>(response.Content.ReadAsStringAsync().Result);
						logData = new Dictionary<string, object> { { "userResult", userResult } };
                        WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "ManageMarketingCenterUser", "Update user. Got result from marketing center." });
                        long newid = userResult.id;
						WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", $"Update user. newid={newid}" });
                        UpdateSamlUserAttribute(userPersonaId, _productId, SamlAttributeEnum.UserId, newid.ToString());
						WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", "Update user success. Saved user id" });

                        if (!IsUserIdValid(Convert.ToInt64(_editorProductUserId)))
						{
							string message = $"ManageMarketingCenterUser.ManageMarketingCenterUser - Invalid admin userId: {_editorProductUserId}";
                            WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", $"Invalid admin userId: {_editorProductUserId}" });
                        }
				...
				{
					// write an error
                    WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "ManageMarketingCenterUser", $"Update user errored. {ex.Message}" });
                    UpdateProductSettingProductStatus(userPersonaId, _productSettingType_ProductStatus, (int)ProductBatchStatusType.Error);
                    WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "ManageMarketingCenterUser", "Update user errored. Set product status to Error." });
                    return "There was a problem updating the user";
				}
			}
			WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "ManageMarketingCenterUser", "Done create/update user" });
		...
		public bool ChangeUserStatus(long editorPersonaId, string username, string productUserId, bool isActive = false)
		{
			...
			{
				WriteToErrorLog("{methodName} - {status}", messageProperties: new object[] { "ChangeUserStatus", $"Error looking for company id in bluebook for user with editorPersona id - {editorPersonaId}" });
                return false;
			}
			try
			{
			...
			{
				WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "ChangeUserStatus", $"Updating user status failed for user {companyInstanceSourceId}|{username} by editorPersonaId = {editorPersonaId}. {ex.Message}" });
                return false;
			}
		}
        #endregion
        ...
        public ListResponse GetRolesCount(long editorPersonaId)
        {
            ...
            catch (Exception ex)
            {
                ...
                WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "GetRolesCount", $"Error {ex.Message}" });
            }
            return response;
        }
        ...
        public ListResponse GetRights(long editorPersonaId)
        {
            ...
            catch (Exception ex)
            {
                response.IsError = true;
                response.ErrorReason = ex.Message;
                WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "GetRights", $"Error {ex.Message}" });
            }
            return response;
        }
        ...
        public ListResponse DeleteRole(long editorPersonaId, int roleId)
        {
            ...
            try
			{
                ...
                if (result.IsSuccessStatusCode)
                {
                    ...
                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "DeleteRole", $"Delete role {roleId}. Got result from marketing center." });
                }
                else
                {
                    WriteToErrorLog("{methodName} - {status}", logData, messageProperties: new object[] { "DeleteRole", $"Delete role {roleId} status errored." });
			...
			{
				WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "DeleteRole", $"Error. {ex.Message}" });
                response = new ListResponse()
				{
					IsError = true,
        ...
        public ListResponse UpdateRoleStatus(long editorPersonaId, int roleId, bool IsActive)
        {
            ...
            try
			{
				...
				if (result.IsSuccessStatusCode)
				{
                    ...
                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateRoleStatus", $"Update roleId {roleId} status. Got result from marketing center." });
                }
				else
				{
					WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "UpdateRoleStatus", $"Update userId {roleId} status errored." });
                    response = new ListResponse()
					{
						IsError = true,
			...
			{
				WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "UpdateRoleStatus", $"Error. {ex.Message}" });
                response = new ListResponse()
				{
					IsError = true,
					ErrorReason = ex.Message
        ...
        public ListResponse UpdateRolesForRight(long editorPersonaId, int rightId, List<string> roleList)
        {
            ...
            try
            {
                ...
                if (result.IsSuccessStatusCode)
                {
                    ...
                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateRolesForRight", $"Update rightId {rightId} status. Got result from marketing center." });
                }
                else
                {
                    WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "UpdateRolesForRight", $"Update rightId {rightId} status errored." });
                    response = new ListResponse()
                    {
                        IsError = true,
            ...
            {
                WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "UpdateRolesForRight", $"Error. {ex.Message}" });
                response = new ListResponse()
                {
                    IsError = true,
                    ErrorReason = ex.Message
        ...
        public ListResponse CreateNewMCRoleWithRights(long editorPersonaId, MCRole mcRole)
        {
            ...
            try
            {
                ...
                if (result.IsSuccessStatusCode)
                {
                    ...
                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "CreateNewMCRoleWithRights", "Got result from marketing center" });
                }
                else
                {
                    WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "CreateNewMCRoleWithRights", "Got error result from marketing center" });
            ...
            {
                WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "CreateNewMCRoleWithRights", $"Error. {ex.Message}" });
                response = new ListResponse()
                {
                    IsError = true,
                    ErrorReason = ex.Message
        ...
        public ListResponse UpdateNewMCRoleWithRights(long editorPersonaId, MCRole mcRole)
        {
            ...
            try
            {
                ...
                if (result.IsSuccessStatusCode)
                {
                    ...
                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateNewMCRoleWithRights", "Got result from marketing center" });
                }
                else
                {
                    WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "UpdateNewMCRoleWithRights", "Got error result from marketing center" });
                    RoleErrors roleErrors = JsonConvert.DeserializeObject<RoleErrors>(result.Content.ReadAsStringAsync().Result);
                    response = new ListResponse()
                    {
                        IsError = true,
            ...
            {
                WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "UpdateNewMCRoleWithRights", $"Error. {ex.Message}" });
                response = new ListResponse()
                {
                    IsError = true,
                    ErrorReason = ex.Message
        ...
        public ListResponse GetRightsForRoleId(long editorPersonaId, int roleId)
		{
            ...
            try
            {
                ...
                WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "GetRightsForRoleId", $"Found blue book company source id {marketingCompanyId}" });
                ...
                WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "GetRightsForRoleId", "Getting rights" });
                ...
                if (apiResponse.IsSuccessStatusCode)
                {
                    ...
                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "GetRightsForRoleId", "Got rights" });
            ...
            {
                WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "GetRightsForRoleId", $"Error. {ex.Message}" });
                response.IsError = true;
        ...
        public ListResponse GetRolesForRightId(long editorPersonaId, int rightId)
        {
            ...
            try
            {
                ...
                WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "GetRolesForRightId", $"Found blue book company source id {marketingCompanyId}" });
                ...
                WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "GetRolesForRightId", "Getting roles" });
                ...
                if (apiResponse.IsSuccessStatusCode)
                {
                    ...
                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "GetRolesForRightId", "Got roles" });
            ...
            {
                WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "GetRolesForRightId", $"Error. {ex.Message}" });
                response.IsError = true;
                response.ErrorReason = "There was a problem getting the roles";
        ...
        private ListResponse GetRolesCountDetails(long editorPersonaId)
        {
            ...
            try
            {
                ...
                WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "GetRolesCountDetails", $"Found blue book company source id {marketingCompanyId}" });
                ...
                WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "GetRolesCountDetails", "Getting roles" });
                ...
                if (apiResponse.IsSuccessStatusCode)
                {
                    ...
                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "GetRolesCountDetails", "Got roles" });
            ...
            {
                WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "GetRolesCountDetails", $"Error. {ex.Message}" });
                response.IsError = true;
                response.ErrorReason = "There was a problem getting the roles";
        ...
        private ListResponse GetRightsDetails(long editorPersonaId)
        {
            ...
            try
            {
                ...
                WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "GetRightsDetails", $"Found blue book company source id {marketingCompanyId}" });
                ...
                WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "GetRightsDetails", "Getting rights" });
                ...
                if (apiResponse.IsSuccessStatusCode)
                {
                    ...
                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "GetRightsDetails", "Got rights" });

                    if (rights == null) { rights = new List<Right>(); }
					else { mcRights = rights.ToGBRights(); }

                    WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "GetRightsDetails", "Returning rights" });
                    response = new ListResponse()
                    {
            ...
            {
                WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "GetRightsDetails", $"Error. {ex.Message}" });
                response.IsError = true;
        ...
        private bool CheckIfUserExistInProduct(string _productUserId)
		{
			...
			if (response.IsSuccessStatusCode)
			{
				WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "CheckIfUserExistInProduct", $"Email address {_productUserId} already exist in Marketing Center." });
                return true;
			}
			return false;
		}
		...
		private bool SetMarketingCenterUserStatus(bool isActive, string mcUserId)
		{
			try
			{
				...
				{
					WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "SetMarketingCenterUserStatus", "Update user status. Editor Product User Id cannot be null or empty." });
                    return false;
				}
				if (mcUserId?.Length > 0 && mcUserId != "0")
				{
					...
					WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "SetMarketingCenterUserStatus", $"Update userId {mcUserId} url and user status request object" });
					...
					if (response.IsSuccessStatusCode)
					{
						WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "SetMarketingCenterUserStatus", $"Update userId {mcUserId} status. Got result from marketing center." });
                        return true;
					}
					else
					{
						WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "SetMarketingCenterUserStatus", $"Update userId {mcUserId} status errored." });
                        return false;
					}
				}
				return false;
			...
			{
                WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "SetMarketingCenterUserStatus", $"Update user status errored. {ex.Message}" });
                return false;
			}
		}
		...
		private T GetResultFromApi<T>(string baseUrlAndQuery) where T : class
		{
			...
			else
			{
				...
				WriteToErrorLog("{methodName} - {status}", logData, messageProperties: new object[] { "GetResultFromApi", "Exiting after error" });
            }
			return results;
		}
		...
		private string ParseErrorPosting(HttpResponseMessage response, string action, long editorPersonaId, long userPersonaId)
		{
			...
			catch (Exception ex)
			{
				...
				WriteToErrorLog("{methodName} - {status}", exception: ex, messageProperties: new object[] { "ManageMarketingCenterUser.ParseErrorPosting", "Error" });
            }
			var logData = new Dictionary<string, object> { { "response.Content.ReadAsStringAsync().Result", userResult } };

            WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "ManageMarketingCenterUser.ParseErrorPosting", $"{action} errored." + (emailAlreadyExists ? " Email already exists" : "") });
            string result = "";
		...
		public ListResponse GetMigrationUsers(long editorPersonaId, RequestParameter datafilter)
		{
			...
			if (companyInstanceSourceId == 0)
			{
				WriteToErrorLog("{methodName} - {status}", messageProperties: new object[] { "GetMigrationUsers", $"Error looking for company id in bluebook for user with editorPersona id - {editorPersonaId}." });
			...
			WriteToDiagnosticLog("{methodName} - {status}", new Dictionary<string, object> { { "Url", url } }, messageProperties: new object[] { "GetMigrationUsers", "Getting users" });

            var migrationResponse = GetResultFromApi<MigrationResponse<IList<MigrationUser>>>(url);

			if (migrationResponse == null)
			{
				WriteToErrorLog("{methodName} - {status}", messageProperties: new object[] { "GetMigrationUsers", $"No users received from product for user with editorPersona id - {editorPersonaId}." });
                return response;
			}
			WriteToDiagnosticLog("{methodName} - {status}", messageProperties: new object[] { "GetMigrationUsers", $"Received users from product for user with editorPersona id - {editorPersonaId}." });
		...
		public MigrateResponse UpdateUsersMigrationStatus(long editorPersonaId, IList<MigrateUser> migrateUsers)
		{
			...
			if (companyInstanceSourceId == 0)
			{
				WriteToErrorLog("{methodName} - {status}", messageProperties: new object[] { "UpdateUsersMigrationStatus", $"Error looking for company id in bluebook for user with editorPersona id - {editorPersonaId}." });
			...
			if (response.IsSuccessStatusCode)
			{
				...
				WriteToDiagnosticLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateUsersMigrationStatus", "PostAsJsonAsync migrate-users" });
			...
			{
				WriteToErrorLog("{methodName} - {status}", logData, messageProperties: new object[] { "UpdateUsersMigrationStatus", "PostAsJsonAsync migrate-users error" });
