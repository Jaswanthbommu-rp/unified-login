    ...
    public class ManageUPFMProductsIntegration : ManageProductBase, IManageUPFMProductsIntegration
    {
        ...
        public ListResponse GetRoles(long editorPersonaId, long userPersonaId, long partyId)
        {
            ...
            try
            {
                ...
                if (result.IsError)
                {
                    WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetRoles", $"GetCompanyEditorAndUserDetails error for product {_upfmProductId} with editorPersona id - {editorPersonaId} - {result.ErrorReason}" });
                    return result;
                }
                ...
                WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetRoles", $"Getting all GB roles from DB - ListRolesForProductByParty with party id - {partyId} and product {_upfmProductId}" });
        ...
        public ListResponse GetUPFMProperties(long editorPersonaId, long userPersonaId, bool assignedOnly, RequestParameter datafilter)
        {
            ...
            WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "GetUPFMProperties", $"Beginning of method for user with editorPersona id - {editorPersonaId} - for product {_upfmProductId}" });
            ...
            catch (Exception ex)
            {
                ...
                WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "GetUPFMProperties", $"There was a problem getting the properties for user with editorPersona id - {editorPersonaId}." },
                    exception: ex);
            }
        ...
        {
            WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "ManageUPFMProductUser", $"Begin create/update user for user with userPersonaId id - {userPersonaId}." });
            try
            {
                var listResponse = GetCompanyEditorAndUserDetails(editorPersonaId, userPersonaId);
                if (listResponse.IsError)
                {
                    WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "ManageUPFMProductUser", $"Error for user with userPersonaId id - {userPersonaId}. Error - {listResponse.ErrorReason}" });
                ...
                {
                    WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "ManageUPFMProductUser", $"New user is Super user with userPersonaId id - {userPersonaId}." });
                ...
                if (userAssignProductPropertyRole != null)
                {
                    ...
                    if (productPropertyRole.RoleList?.Count > 0)
                    {
                        ...
                        if (existinguserRoleIds.Count > 0)
                        {
                            foreach (var item in existinguserRoleIds.ToList())
                            {
                                ...
                                if (result.Id < 0)
                                {
                                    WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "ManageUPFMProductUser", $"Unable to delete role for user with userPersonaId - {userPersonaId}, RoleId - {item}" });
                        ...
                        if (userassignedRoles.Count > 0)
                        {
                            ...
                            {
                                //add the role
                                WriteToDiagnosticLog("{methodName} - {state}", messageProperties: new object[] { "ManageUPFMProductUser", $"Adding role for userPersonaId id - {userPersonaId}, RoleId - {item}." });
                                ...
                                if (result.Id < 0)
                                {
                                    WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "ManageUPFMProductUser", $"Unable to add role for user with userPersonaId - {userPersonaId}, RoleId - {item}" });
                                    return result.ErrorMessage;
                                }
                            }
                    ...
                    if (userAssignProductPropertyRole.PropertyList != null && userAssignProductPropertyRole.PropertyList.Count > 0)
                    {
                        ...
                        if (!IsSuperUser(userPersonaId) && userAssignProductPropertyRole.IsAssigned && assignedPropertyList?.Count == 0 && unassignedProperties?.Count == 0)
                        {
                            ...
                            {
                                WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "ManageUPFMProductUser", $"No Properties are found to assign/unassign for user with userPersonaId - {userPersonaId}" });
            ...
            {
                WriteToErrorLog("{methodName} - {state}", messageProperties: new object[] { "ManageUPFMProductUser", $"Error for user with userPersonaId id - {userPersonaId}" }, exception: ex);
        ...
        public string UnassignUser(long editorPersonaId, long userPersonaId, UPFMProductPropertyRole userAssignProductPropertyRole)
        {
            ...
            WriteToInformationLog("{methodName} - {state}", messageProperties: new object[] { "UnassignUser", $"userPersonaId:{userPersonaId} unassigned" });
