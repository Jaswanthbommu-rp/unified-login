        ...
        public void ReadMqActivity(MsmqMessage<ActivityDetailMessage> mqMessage)
        {
            ...
            try
            {
                ...
                if (activity.OrganizationPartyId == 0)
                {
                    ...
                    logger.Write(LogEventLevel.Error, messageTemplate: "{methodName} - {state}", propertyValues: new object[] { "ReadMqActivity", $"Activity Message with no organization party id. Message -{activity?.Message}" });
                }
            ...
            {
                Log.Error(ex, messageTemplate: "{methodName} - {state}", propertyValues: new object[] { "ReadMqActivity", $"Error. User: {activity?.FromUserLoginId.ToString()}. PmcId: {activity?.OrganizationPartyId.ToString()} Reason: {ex.Message}" });
            }
        }
    }
}
